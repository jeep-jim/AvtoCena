...
import {displaySpecificationGroups} from '../apps/web/lib/catalog/specification-display.ts';
...
for (const market of ['korea', 'china', 'uae', 'europe', 'georgia', 'japan']) {
  ...
  for (const chunk of manifest.markets?.[market]?.chunks || []) {
    ...
    for (const row of rows) {
      stats.rows++;
      if(market==='korea')for(const group of displaySpecificationGroups(row.operational?.sourceSpecifications?.groups||[]))for(const item of group.items)for(const text of [item.name,item.value])if(/[\p{Script=Hangul}]/u.test(text)){const existing=untranslated.get(text)||{text,count:0,example:row.id};existing.count++;untranslated.set(text,existing);}
...
console.log('KOREAN_UNTRANSLATED '+JSON.stringify([...untranslated.values()].sort((a,b)=>b.count-a.count).slice(0,100)));
for(const pool of samples.values()){
 const row=pool[0];const url=row.imageUrls?.[0];if(!url)continue;
 const result={sourceId:row.sourceId,id:row.id};
 try{
 const u=new URL(url);if(u.protocol!=='https:'||u.username||u.password||u.port||!/^[a-z0-9.-]+$/i.test(u.hostname)||/^(localhost|127\.|10\.|192\.168\.|169\.254\.)/.test(u.hostname))throw Error('invalid_image_host');
 result.host=u.hostname;
 const response=await fetch(url,{headers:{Range:'bytes=0-65535'},signal:AbortSignal.timeout(15000),redirect:'error'});
 result.status=response.status;result.contentType=response.headers.get('content-type');await response.body?.cancel();
 }catch(error){result.error=String(error.message);}console.log('PHOTO_SAMPLE '+JSON.stringify(result));
}
