...
async function main() {
 ...
 if (process.env.REPAIR_PILOT_ID) {
  ...
  if (old.instanceId !== process.env.REPAIR_PILOT_ID || vm.id !== old.instanceId || vm.labels?.app !== "avtocena-browser" || old.releaseSha !== process.env.REPAIR_RELEASE_SHA || old.expiresAt <= Date.now()) throw Error("repair_identity_guard");
  ...
  yc(["compute","instance","stop","--id",vm.id]);
