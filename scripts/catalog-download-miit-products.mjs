...
const MAX_PAGES=Math.max(1,Math.min(500,Number(process.env.KNOWLEDGE_MIIT_MAX_PAGES_PER_BATCH||100)));
...
for(let batch=FIRST;batch<=LAST;batch++){
 const products=[];let page=1,totalPages=1,totalExpected=null,status="complete";
 try{
  ...
  if(totalPages>MAX_PAGES){status="partial_page_cap";summary.status="partial";summary.errors.push({batch,error:`page_cap:${MAX_PAGES}/${totalPages}`});}
  if(status==="complete")summary.counts.batchesComplete++;
 ...
 const text=JSON.stringify(payload,null,2)+"\n";await fs.writeFile(path.join(OUT,"products",`batch-${batch}.json`),text);summary.batches[String(batch)]={status,products:products.length,totalExpected,sha256:sha256(text)};summary.counts.products+=products.length;
...
if(summary.counts.products===0&&summary.counts.blockedBatches===0)throw new Error("miit_products_zero_without_access_block");
