...
import { ChnGoodCarPaginatedExactAdapter, goodCarIdentityNamedElectrifiedKind, type GoodCarPaginatedExactRawOffer } from '../apps/web/lib/catalog/chngoodcar-paginated-exact-source';
...
const OUTPUT_PATH = process.env.CATALOG_SOURCE_CHNGOODCAR_LIST_EXHAUSTION_OUTPUT || 'catalog-source-chngoodcar-list-exhaustion-v1.json';
...
export function stratifiedGoodCarPages(pageCount: number, wanted = 6) {
  const total = Math.max(1, Math.trunc(pageCount || 1));
  const count = Math.max(1, Math.min(total, Math.trunc(wanted || 1)));
  if (count === 1) return [1];
  const pages = new Set<number>();
  for (let i = 0; i < count; i += 1) {
    pages.add(Math.max(1, Math.min(total, Math.round(1 + ((total - 1) * i) / (count - 1)))));
  }
...
function exactRejectionReasons(row: GoodCarPaginatedExactRawOffer) {
...
export async function runGoodCarListExhaustion() {
  ...
  const exactAdapter = new ChnGoodCarPaginatedExactAdapter();
  const accepted: any[] = [];
  const rejected: any[] = [];
  ...
  for (const page of samplePages) {
    const result = await exactAdapter.fetchPage(String(page));
    ...
    for (const row of rawRows) {
      ...
      const offer = exactAdapter.normalizeOffer(row);
      if (namedElectrified) {
      ...
      if (!offer) {
        rejected.push({ page, sourceOfferId: row.sourceOfferId, sourceTitle: row.sourceTitle, reasons: exactRejectionReasons(row) });
        continue;
      }
      const images = await exactAdapter.fetchImages(offer);
  ...
  const suspiciousLowPriceAccepted = accepted.filter((row) => Number(row.year) >= 2020 && Number(row.sourcePrice) < 2000);
  ...
  const checks = {
    ...
    noSuspicious2020PlusSub2000Accepted: suspiciousLowPriceAccepted.length === 0,
  };
  ...
  const payload = {
    ...
    mode: 'chngoodcar_full_list_exhaustion_plus_stratified_detail_no_write',
    ...
    coverage: {
      ...
      acceptedMakes,
      acceptedPages,
      suspicious2020PlusSub2000AcceptedCount: suspiciousLowPriceAccepted.length,
    },
    ...
    suspicious2020PlusSub2000Accepted: suspiciousLowPriceAccepted.slice(0, 20),
    checks,
    failures,
    next: failures.length
      ? 'repair the exact gate or list-exhaustion assumptions from this no-write evidence before source promotion'
      : 'full list pagination is exhausted and stratified exact sampling is green; keep publishAllowed=false until manual review and a separate production-promotion decision',
  };
  ...
  console.log(JSON.stringify({
    ...
    acceptedMakes,
    suspicious2020PlusSub2000Accepted: suspiciousLowPriceAccepted.length,
    failures,
  }, null, 2));
  if (failures.length) throw new Error(`chngoodcar_list_exhaustion_failed:${failures.join(',')}`);
  return payload;
}

if (import.meta.url === new URL(process.argv[1], 'file:').href) {
  runGoodCarListExhaustion().catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
}
