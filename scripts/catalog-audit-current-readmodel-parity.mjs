...
import { auditStableCatalogSnapshot } from "./lib/catalog-stable-parity.mjs";
...
const report = await auditStableCatalogSnapshot({
  audit: auditSnapshot,
  readManifest: () => readDataJson("catalog/manifest.json", {generationId: ""}),
  readLock: () => readDataJson("catalog/import-lock.json", {}),
  onRetry: state => console.log(JSON.stringify({event: "catalog_parity_retry", ...state})),
}).catch(async error => {
  if (error.report) await fs.writeFile(OUTPUT, JSON.stringify({...error.report, ok: false, auditError: error.message}, null, 2) + "\n", "utf8");
  throw error;
});
