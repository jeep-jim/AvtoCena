...
check("korea_registration",korea,v=>{const c=v.counts||{},accepted=Number(c.rowsIn2020Plus||0),explicit=Number(c.rowsWithExplicitModelYear||0),registration=Number(c.rowsWithFirstRegistrationWindowOnly||0),cache=v.cache;const cacheOk=!cache?.used||(Number(cache.provenance?.workflowRunId||0)===32557200820&&cache.verification?.rawSha256===v.raw?.sha256&&Number(cache.verification?.uniqueTuples||0)===Number(c.uniqueTuples||0)&&Number(cache.verification?.observations||0)===accepted&&v.raw?.compactedStoredInGit===true);return Number(c.rowsParsed||0)>=100000&&accepted>=1000&&Number(c.uniqueTuples||0)>=1000&&accepted===explicit+registration&&cacheOk;},"rows>=100000 accepted 2020+ evidence>=1000 unique tuples>=1000 and every accepted row has an explicit model-year or first-registration-year basis; any cached compact fallback must match its pinned workflow provenance, raw hash and reconciled counts");
