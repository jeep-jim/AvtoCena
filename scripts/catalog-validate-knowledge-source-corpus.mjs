...
check("miit_china_products",miit,v=>Number(v.counts?.products||0)>=1000&&Number(v.counts?.batchesComplete||0)>=50&&Number(v.counts?.blockedBatches||0)===0,"MIIT products>=1000 complete batches>=50 no access block");
...
check("korea_registration",korea,v=>Number(v.counts?.rowsParsed||0)>=100000&&Number(v.counts?.uniqueTuples||0)>=1000,"rows>=100000 unique tuples>=1000");
