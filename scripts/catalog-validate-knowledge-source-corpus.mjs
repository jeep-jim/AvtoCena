...
check("miit_china_products",miit,v=>Number(v.counts?.products||0)>=1000&&Number(v.counts?.batchesComplete||0)===Number(v.counts?.batchesRequested||-1)&&Number(v.counts?.blockedBatches||0)===0&&Number(v.counts?.failedBatches||0)===0,"MIIT products>=1000 and every requested batch complete with no block/failure");
