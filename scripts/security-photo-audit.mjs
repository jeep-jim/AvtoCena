...
if(process.env.YC_SA_JSON_CREDENTIALS){
 try{
  ...
  for(const c of (containers.containers||[]).filter(x=>x.name==='avtocena-web')){
   ...
   report.cloud.runtime=(revisions.revisions||[]).filter(x=>x.status==='ACTIVE').map(x=>({release:x.image?.environment?.AVTOCENA_RELEASE_SHA,authSecretPresent:Boolean(x.image?.environment?.AUTH_SECRET||x.image?.environment?.NEXTAUTH_SECRET),captchaClientPresent:Boolean(x.image?.environment?.SMARTCAPTCHA_CLIENT_KEY),captchaServerPresent:Boolean(x.image?.environment?.SMARTCAPTCHA_SERVER_KEY),imageStorageMode:x.image?.environment?.CATALOG_IMAGE_STORAGE_MODE||'source_urls_only (code default)',resources:x.resources,concurrency:x.concurrency,executionTimeout:x.executionTimeout,provisionPolicy:x.provisionPolicy,scalingPolicy:x.scalingPolicy}));
  }
  const gateways=await get('https://serverless-apigateway.api.cloud.yandex.net/apigateways/v1/apigateways?folderId='+folder);report.cloud.gateways={status:gateways.auditHttpStatus||200,items:(gateways.apiGateways||[]).map(x=>({name:x.name,status:x.status,customDomains:x.customDomains?.map(y=>y.fqdn)}))};
  for(const gateway of (gateways.apiGateways||[]).filter(x=>x.name==='avtocena-gateway')){const spec=await get('https://serverless-apigateway.api.cloud.yandex.net/apigateways/v1/apigateways/'+gateway.id+':spec');report.cloud.gatewaySecurityBinding={status:spec.auditHttpStatus||200,hasSmartWebSecurity:typeof spec.openapiSpec==='string'?/smartWebSecurity/.test(spec.openapiSpec):null};}
