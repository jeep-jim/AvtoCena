...
export async function collectSourcePage(state, options) {
  ...
  try { page = await state.source.fetchPage(state.cursor); }
    ...
    const transient=!blocked && /timeout|timed.?out|abort|network|fetch failed|econnreset|http[_: ]5\d\d|yandex_bridge_non_json_5\d\d|transient(?:[_: ]status)?[_: ]202/i.test(message);
