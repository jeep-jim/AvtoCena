...
export function isExistingPilotBridgeRequest(url, method, sourceId, pageLimit = 1) {
  ...
  const page = url.searchParams.get(['myauto_georgia_list', 'autopapa_georgia_open'].includes(sourceId) ? 'startPage' : 'page');
  ...
  if (sourceId === 'dubizzle_uae_open') return match('/api/internal/dubizzle-egress-a4c907', [['page', page]]);
  if (sourceId === 'myauto_georgia_list') return match('/api/internal/georgia-recovery-e2f913', [['pages', '1'], ['source', 'myauto'], ['startPage', page]]);
  if (sourceId === 'autopapa_georgia_open') return match('/api/internal/georgia-recovery-e2f913', [['pages', '1'], ['source', 'autopapa'], ['startPage', page]]);
  return false;
}
