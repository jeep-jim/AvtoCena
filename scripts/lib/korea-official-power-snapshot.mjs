...
export function buildOfficialPowerSnapshot(rows,previous,{capturedAt,rawSha256,unitHtml}) {
 if(!Array.isArray(rows)||rows.length<3000||rows.length<(previous.sourceRowCount||previous.records.length)*0.9)throw Error('official_power_snapshot_collapse');
 ...
 return {...previous,capturedAt,rawSha256,sourceRowCount:rows.length,previousSnapshotSha256:previous.rawSha256,
