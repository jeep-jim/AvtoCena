...
const { readMarketOffers, readAllOffersForMaintenance } = await import("../apps/web/lib/catalog/storage.ts");
const { hasCredibleCatalogIdentity, isCatalogYearAllowed, isCatalogMarketSourceAllowed } = await import("../apps/web/lib/catalog/offer-quality.ts");
...
const OUTPUT = process.env.ISSUE241_IDENTITY_PLAN_OUTPUT || "issue241-invalid-identity-migration-plan.json";
const CHINA = new Map([
...
function hashRows(rows) {
  const canonical = [...rows].sort((a, b) => String(a?.id || "").localeCompare(String(b?.id || ""))).map(stable);
...
function renderedProblems(offer) {
  const p = presentCatalogOffer(offer);
  const makeLabel = String(p?.makeLabel || "").trim(), modelLabel = String(p?.modelLabel || "").trim(), title = String(p?.title || "").trim();
...
function assertExact(condition, message) { if (!condition) throw new Error(message); }
...
for (const market of PUBLIC_CATALOG_MARKETS) {
  const before = publicBefore[market], rows = publicAfter[market];
  ...
  const invalidIdentity = rows.filter((offer) => !hasCredibleCatalogIdentity(offer)).length;
  const rendered = rows.filter((offer) => renderedProblems(offer).length).length;
  const belowYear = rows.filter((offer) => !isCatalogYearAllowed(offer?.year, market)).length;
  const invalidSource = rows.filter((offer) => !isCatalogMarketSourceAllowed(offer)).length;
  const nonVehicleCount = rows.filter((offer) => nonVehicle.test(`${offer?.make || ""} ${offer?.model || ""} ${offer?.trim || ""} ${offer?.bodyType || ""}`)).length;
  const badPrice = rows.filter((offer) => !(Number(offer?.sourcePrice || 0) > 0) || !String(offer?.sourceCurrency || "").trim()).length;
  const belowFive = rows.filter((offer) => !Array.isArray(offer?.images) || offer.images.length < 5).length;
  const maxModelYear = modelYearCounts.size ? Math.max(...modelYearCounts.values()) : 0;
  reportMarkets[market] = { before: before.length, after: rows.length, beforeHash: hashRows(before), afterHash: hashRows(rows), invalidIdentity, renderedIdentity: rendered, belowYear, invalidSource, nonVehicleCount, badPrice, belowFive, maxModelYear };
  if (invalidIdentity) failures.push(`${market}:invalid_identity:${invalidIdentity}`);
  if (rendered) failures.push(`${market}:rendered_identity:${rendered}`);
  if (belowYear) failures.push(`${market}:year:${belowYear}`);
  if (invalidSource) failures.push(`${market}:source:${invalidSource}`);
  if (nonVehicleCount) failures.push(`${market}:non_vehicle:${nonVehicleCount}`);
  if (badPrice) failures.push(`${market}:source_price:${badPrice}`);
  if ((market === "korea" || market === "japan") && belowFive) failures.push(`${market}:below_five:${belowFive}`);
  if (maxModelYear > CATALOG_MAX_OFFERS_PER_MODEL_YEAR) failures.push(`${market}:model_year:${maxModelYear}`);
}
for (const market of ["korea", "japan", "uae"]) {
  if (reportMarkets[market].beforeHash !== reportMarkets[market].afterHash) failures.push(`${market}:unexpected_hash_change`);
}
for (const market of ["china", "georgia", "kyrgyzstan"]) {
  if (reportMarkets[market].before !== reportMarkets[market].after) failures.push(`${market}:unexpected_count_change`);
}
if (reportMarkets.europe.after !== reportMarkets.europe.before - 2) failures.push(`europe:unexpected_count_delta:${reportMarkets.europe.before}:${reportMarkets.europe.after}`);
const internalAfterIds = new Set(internalAfter.map((offer) => String(offer?.id || "")));
for (const id of EUROPE_DROP.keys()) if (internalAfterIds.has(id)) failures.push(`europe_drop_internal_retained:${id}`);

const report = {
  version: 1,
  mode: "issue241_invalid_identity_migration_plan",
  checkedAt: new Date().toISOString(),
  expectedChanges: { chinaModelRepairs: CHINA.size, europeDrops: EUROPE_DROP.size, georgiaModelRepairs: GEORGIA.size, kyrgyzstanModelRepairs: KYRGYZSTAN.size },
  internalBefore: internalBefore.length,
  internalAfter: internalAfter.length,
