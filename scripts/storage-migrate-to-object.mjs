...
for (const file of binaryFiles) { const rel = relative(dataRoot, file).replaceAll('\\','/'); if (!dryRun) { try { const data = readFileSync(file); if (!force && await storage.exists(rel)) { binary.skipped_existing += 1; continue; } await storage.putBinary(rel, data, mime(file)); const uploaded = await storage.getBinary(rel); if (uploaded.size !== statSync(file).size) binary.errors.push({ file: rel, error: 'verify_failed' }); } catch (error) { binary.errors.push({ file: rel, error: error instanceof Error ? error.message : 'upload_failed' }); } } }
report.push(binary);
