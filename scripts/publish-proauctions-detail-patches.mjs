...
try{
 ...
 let patched=0,sheetsAdded=0,powerRestored=0;
 const expected=all.map(row=>{
  if(row.market!=='japan' || row.sourceId!=='proauctions_japan_stat')return row;
  const patch=patches.get(row.id);if(!patch)throw Error(`missing_patch:${row.id}`);
 ...
 validate(expected);
 const report={published:false,operationId,previousGeneration:previous.generationId,patched,sheetsAdded,powerRestored,otherMarketHashes:Object.fromEntries(Object.entries(preserved).map(([m,r])=>[m,hashCatalogRows(r)]))};
