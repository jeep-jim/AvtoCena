...
for (const url of candidates) {
  try {
    ...
    reports.push({ url, status: 0, error: String(error?.message || error), matches: [] });
  }
}
...
console.log(JSON.stringify({ id, category: report.category, endpoints: reports.map((r) => ({ url: r.url, status: r.status, matches: r.matches.length })) }, null, 2));
