...
for (const market of markets) {
  ...
  for (const raw of rows) {
    ...
    if (/Марка уточняется|Модель уточняется/i.test(presented.title || '')) {
      unresolvedTitle++;
      if (unresolved.length<100) unresolved.push({market,id:offer.id,sourceId:offer.sourceId,make:offer.make,model:offer.model,sourceTitle:offer.sourceTitle,trim:offer.trim,presentedTitle:presented.title,sourceUrl:offer.operational?.sourceUrl});
    }
    ...
    if (sourceRub && total) {
      ...
      if (ratio>=4 || total>=15_000_000) priceOutliers.push({market,id:offer.id,sourceId:offer.sourceId,make:offer.make,model:offer.model,year:offer.year,sourcePrice:offer.sourcePrice,sourceCurrency:offer.sourceCurrency,sourceRub,totalRub:total,ratio,calculationStatus:offer.calculationStatus,pricingConfidence:offer.calculationSnapshot?.pricingConfidence,sourceUrl:offer.operational?.sourceUrl});
    }
  }
...
const output={
  ...
  missing30Groups:[...powerGroups.values()].sort((a,b)=>b.count-a.count).slice(0,250).map((x)=>({...x,sourceIds:[...x.sourceIds],peakPowersHp:[...x.peakPowersHp]})),
  repeatedJapanImages,
  priceOutliers:priceOutliers.sort((a,b)=>b.ratio-a.ratio || b.totalRub-a.totalRub).slice(0,250),
};
