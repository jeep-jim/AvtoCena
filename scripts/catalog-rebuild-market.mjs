...
const { cacheImageFromUrl } = await import("../apps/web/lib/catalog/storage.ts");
...
const maxImages = Math.min(120, Math.max(1, Number(process.env.CATALOG_MAX_IMAGES_PER_OFFER || 120)));
const maxPages = Math.max(1, Number(process.env.CATALOG_REBUILD_MAX_PAGES || 240));
const outputFile = process.env.CATALOG_REBUILD_OUTPUT || `catalog-rebuild-${market}.json`;
const timeoutMs = Math.max(10_000, Number(process.env.CATALOG_SOURCE_TIMEOUT_MS || 45_000));
const imageConcurrency = Math.max(1, Math.min(10, Number(process.env.CATALOG_IMAGE_FETCH_CONCURRENCY || 6)));

const sourcePlan = {
  korea: ["encar_direct"],
  china: ["che168_china_exact", "guazi_china_export"],
  japan: ["goonet_japan_exact"],
...
function imageIdentity(image) { return String(image?.checksum || image?.id || image?.objectKey || image?.url || ""); }
...
function collapseRepeatedPhrases(value) {
  ...
  while (changed) {
    changed = false;
    const maxPhrase = Math.min(10, Math.floor(tokens.length / 2));
...
function removeLeading(value, phrase) {
  ...
  return sourceLower === prefixLower ? "" : sourceLower.startsWith(`${prefixLower} `) ? source.slice(prefix.length).trim() : source;
}
...
function absoluteUrl(value, base) {
  try { return new URL(text(value), base).toString(); } catch { return ""; }
}
function imageUrl(value, base) {
  const url = absoluteUrl(value, base);
  if (!url || badImage.test(url)) return "";
  return /\.(?:jpe?g|png|webp|avif)(?:[?#]|$)/i.test(url) || /olxcdn\.com|ci\.encar\.com|picture\d*\.goo-net\.com|\/image(?:[;/?#]|$)/i.test(url) ? url : "";
}
...
async function requestJson(url, referer = url) {
  ...
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetch(url, { headers: { ...headers, accept: "application/json,text/plain,*/*", referer }, redirect: "follow", signal: controller.signal });
...
function deepStrings(value, output = [], depth = 0) {
  ...
  if (typeof value === "string") {
    ...
    if (/^https?:|^\/\/|^\/?carpicture\//i.test(candidate) && /(?:image|photo|picture|carpicture|\.(?:jpe?g|png|webp)(?:[?#]|$))/i.test(candidate)) output.push(candidate);
    return output;
  }
  if (Array.isArray(value)) { value.forEach((item) => deepStrings(item, output, depth + 1)); return output; }
  if (typeof value === "object") Object.values(value).forEach((item) => deepStrings(item, output, depth + 1));
  return output;
}
function deepNumber(value, keys, depth = 0) {
  if (value == null || depth > 14 || typeof value !== "object") return undefined;
  if (Array.isArray(value)) {
    for (const item of value) { const result = deepNumber(item, keys, depth + 1); if (result) return result; }
    return undefined;
  }
  ...
  for (const item of Object.values(value)) { const result = deepNumber(item, keys, depth + 1); if (result) return result; }
  return undefined;
}
function rawImageValues(offer) {
  ...
  for (const field of [raw?.images, raw?.photos, raw?.gallery, raw?.imageUrls, raw?.photoUrls]) {
    ...
    for (const item of field) {
      ...
      else if (item && typeof item === "object") for (const key of ["url", "src", "path", "location", "large", "original"]) if (typeof item[key] === "string") result.push(item[key]);
    }
  }
  return result;
}
function collectHtmlImages(markup, pageUrl) {
  const values = [];
  const decoded = text(markup).replace(/\\u002f/gi, "/").replace(/\\u003a/gi, ":").replace(/\\u0026/gi, "&");
  for (const match of decoded.matchAll(/<(?:img|source)[^>]+(?:data-src|data-original|data-lazy-src|src|content)\s*=\s*["']([^"']+)["']/gi)) values.push(match[1]);
  for (const match of decoded.matchAll(/(?:srcset|data-srcset)\s*=\s*["']([^"']+)["']/gi)) for (const part of match[1].split(",")) values.push(part.trim().split(/\s+/)[0]);
  for (const match of decoded.matchAll(/https?:\/\/[^"'<>\s\\]+(?:\.(?:jpe?g|png|webp|avif)|\/image(?:[;/?#]|$))[^"'<>\s\\]*/gi)) values.push(match[0]);
...
async function discoverKoreaUrls(offer) {
  ...
  const count = Math.min(maxImages, Math.max(0, deepNumber(detail, ["photoCount", "PhotoCount", "imageCount", "ImageCount", "pictureCount", "PictureCount"]) || 0));
  if (cover) {
    for (let index = 1; index <= (count || maxImages); index++) {
      if (/^https?:\/\//i.test(cover) && /(\d{3})(\.(?:jpe?g|png|webp))$/i.test(cover)) values.push(cover.replace(/(\d{3})(\.(?:jpe?g|png|webp))$/i, `${String(index).padStart(3, "0")}$2`));
      else values.push(buildEncarImageUrl(cover, index));
    }
  }
  return uniqueUrls(values.map((value) => /^https?:\/\//i.test(value) ? value : buildEncarImageUrl(value, 1)).filter((value) => /ci\.encar\.com|\/carpicture\//i.test(value)));
}
...
async function discoverDetailUrls(offer) {
  ...
  if (market === "uae") {
    values = values.filter((value) => /\/images\/[a-f0-9]{6}\/(?:w_?\d+x\d+|\d+x\d+|f_?\d+x\d+)\/[^/?#]+\/[a-f0-9-]+\.(?:jpe?g|webp)/i.test(value) && !/\/(?:130x76|f_500x282)\//i.test(value));
  }

  const rawValues = rawImageValues(offer).map((value) => imageUrl(value, sourceUrl)).filter(Boolean);
  ...
  return uniqueUrls((preferred.length >= 2 ? preferred : best.length >= 2 ? best : rawValues).filter(Boolean));
}
...
async function cacheUrls(urls, offer) {
  ...
  for (let index = 0; index < urls.length && result.length < maxImages; index += imageConcurrency) {
    const batch = await Promise.all(urls.slice(index, index + imageConcurrency).map((url) => cacheImageFromUrl(url, market, { headers: { ...headers, referer } }).catch(() => null)));
...
for (const sourceId of sourceIds) {
  ...
  if (!source) throw new Error(`catalog_source_not_found_${sourceId}`);
  let cursor = null;
  let pages = 0;
  ...
  while (offers.size < target && pages < maxPages) {
    ...
    try {
      ...
      consecutiveErrors++;
      report.sourceErrors.push({ sourceId, cursor, error: String(error?.message || error) });
    ...
    for (const raw of rows) {
      ...
      report.seen++;
      let offer;
      try { offer = source.normalizeOffer(raw); } catch { offer = null; }
      if (!offer || offer.market !== market || !offer.id || offers.has(offer.id)) { report.skipped++; continue; }

      offer = cleanOfferText(normalizeVehicleOfferSpecs(offer));
      let nativeImages = [];
      try { nativeImages = credibleCatalogImages(await source.fetchImages(offer)); }
      catch (error) { report.imageFailures++; report.sourceErrors.push({ sourceId, offerId: offer.id, error: String(error?.message || error) }); }
      ...
      try {
        ...
        report.sourceErrors.push({ sourceId, offerId: offer.id, stage: "gallery_expansion", error: String(error?.message || error) });
      }

      const images = mergeImages(nativeImages, extraImages);
      if (!images.length) { report.imageFailures++; report.skipped++; continue; }
      ...
      try { offer = cleanOfferText(normalizeVehicleOfferSpecs(await calculateOffer(offer))); }
      catch (error) { report.sourceErrors.push({ sourceId, offerId: offer.id, stage: "calculation", error: String(error?.message || error) }); report.skipped++; continue; }

      if (!isCrediblePublicOffer(offer)) { report.rejected++; continue; }
      offers.set(offer.id, offer);
      report.saved = offers.size;
      if (offers.size % 10 === 0) console.log(`[fresh:${market}] ${offers.size}/${target}; ${sourceId}; photos=${images.length}`);
    }
...
report.saved = offers.size;
report.publicBySource = [...offers.values()].reduce((totals, offer) => { totals[offer.sourceId] = (totals[offer.sourceId] || 0) + 1; return totals; }, {});
...
await fs.writeFile(outputFile, JSON.stringify({ version: 1, market, generatedAt: report.finishedAt, target, count: offers.size, sourceIds, report, offers: [...offers.values()].slice(0, target) }, null, 2));
