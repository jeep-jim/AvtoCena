...
const NON_PUBLIC_CANONICAL_SCRIPTS = /[\p{Script=Han}\p{Script=Hangul}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Cyrillic}\p{Script=Arabic}]/u;
...
function aliasRows(entity, entityType, indexes) {
  ...
  const canonicalSafe = ["seed", "verified"].includes(entity.status);
  const values = canonical ? [{ value: canonical, kind: "canonical", safe: canonicalSafe, sourceIds: [] }] : [];
  values.push(...(entity.aliases || []), ...(entity.sourceNames || []));
  return values.map((alias) => ({
    normalized: normalizeTerm(alias.value),
    value: alias.value,
    kind: alias.kind,
    safe: canonicalSafe && alias.safe,
    entityType,
    entityId: entity.id,
...
export async function validateWorkspace({ root = WORKSPACE_ROOT, writeReports = false } = {}) {
  ...
  for (const brand of data.records.brand) {
    if (NON_PUBLIC_CANONICAL_SCRIPTS.test(brand.canonicalName)) {
      issue(errors, "brand.canonical_non_public_script", "Brand canonicalName must use the stable public English/Latin identity; localized scripts belong in aliases", { entityId: brand.id, canonicalName: brand.canonicalName });
    }
    if (brand.status === "verified" && !brand.countries.length) {
      issue(errors, "brand.country_missing", "A verified brand must have at least one source-backed country", { entityId: brand.id });
    }
  }
  ...
  for (const model of data.records.model) {
    ...
    if (!covers.length) {
      const target = ["seed", "verified"].includes(model.status) ? errors : warnings;
      issue(target, "media.cover_missing", "Model must reference an approved canonical cover with verified identity before publication", { entityId: model.id, status: model.status });
    }
  }
  ...
  for (const media of data.records.media) {
    ...
    if (media.role === "brand_logo") {
      const owner = indexes.brands.get(media.ownerId);
      const themeKey = `${media.ownerId}:${media.theme}`;
      if (brandLogoThemes.has(themeKey)) issue(errors, "brand_logo.duplicate_theme", "A brand may have only one canonical logo per theme", { entityId: media.id, previousId: brandLogoThemes.get(themeKey), ownerId: media.ownerId, theme: media.theme });
      else brandLogoThemes.set(themeKey, media.id);
      if (owner && media.assetPath !== `assets/brand-logos/${media.theme}/${owner.slug}.png`) {
        issue(errors, "brand_logo.path_owner_mismatch", "Brand logo path must match its owner slug and theme", { entityId: media.id, ownerId: media.ownerId, assetPath: media.assetPath });
      }
      if (media.status === "approved" && media.rightsStatus !== "cleared") {
        issue(errors, "brand_logo.rights_not_cleared", "An approved brand logo must have cleared source/rights review", { entityId: media.id, rightsStatus: media.rightsStatus });
      }
      const assetFile = path.resolve(root, media.assetPath || "");
      const relativeAsset = path.relative(root, assetFile);
      if (relativeAsset.startsWith("..") || path.isAbsolute(relativeAsset)) {
        issue(errors, "brand_logo.path_outside_workspace", "Brand logo assetPath must stay inside the V2 workspace", { entityId: media.id, assetPath: media.assetPath });
        continue;
      }
      try {
        const buffer = await readFile(assetFile);
        const png = pngInfo(buffer);
        if (!png || png.width !== 90 || png.height !== 60 || ![4, 6].includes(png.colorType)) {
          issue(errors, "brand_logo.format", "Brand logo must be a transparent PNG on an exact 90x60 canvas", { entityId: media.id, assetPath: media.assetPath, png });
        }
        const checksum = createHash("sha256").update(buffer).digest("hex");
        if (checksum !== media.sha256) issue(errors, "brand_logo.checksum", "Brand logo checksum does not match the staged asset", { entityId: media.id, assetPath: media.assetPath });
      } catch {
        issue(errors, "brand_logo.asset_missing", "Brand logo assetPath does not exist", { entityId: media.id, assetPath: media.assetPath });
      }
    }
  }
  ...
  const missingBrands = data.records.brand
    .map((brand) => ({ entityId: brand.id, missing: brand.countries.length ? [] : ["countries"] }))
  ...
  const reports = {
    ...
    missingFields: { brands: missingBrands, variants: missingFields },
  ...
  if (writeReports) {
    ...
    const verifiedBrands = data.records.brand.filter((row) => row.status === "verified").map((row) => row.canonicalName);
    const verifiedBrandNames = new Set(verifiedBrands);
    const completedBrands = [...new Set(Array.isArray(manifest.completedBrands) ? manifest.completedBrands : [])]
      .filter((name) => verifiedBrandNames.has(name));
    const completedBrandNames = new Set(completedBrands);
    manifest.verifiedBrands = verifiedBrands;
    manifest.completedBrands = completedBrands;
    manifest.inProgressBrands = data.records.brand
      .filter((row) => !completedBrandNames.has(row.canonicalName))
    ...
    manifest.status = errors.length
      ? "invalid"
      : manifest.inProgressBrands.length
        ? "checkpoint-seed"
