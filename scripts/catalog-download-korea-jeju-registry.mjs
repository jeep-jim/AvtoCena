...
const map=new Map();let accepted=0;
for(const row of parsed){const name=clean(row[ix.name]);if(!name)continue;const year=ix.year>=0?Number(String(row[ix.year]||"").replace(/[^0-9]/g,"")):null;if(year&&year<2020)continue;const tuple={vehicleName:name,modelYear:Number.isInteger(year)&&year>=2020&&year<=2026?year:null,engineCc:ix.cc>=0?Number(String(row[ix.cc]||"").replace(/[^0-9.]/g,""))||null:null,fuel:ix.fuel>=0?clean(row[ix.fuel])||null:null,vehicleClass:ix.class>=0?clean(row[ix.class])||null:null,vehicleType:ix.type>=0?clean(row[ix.type])||null:null,vehicleKind:ix.kind>=0?clean(row[ix.kind])||null:null,purpose:ix.purpose>=0?clean(row[ix.purpose])||null:null};const key=JSON.stringify(tuple);const cur=map.get(key)||{...tuple,observations:0};cur.observations++;map.set(key,cur);accepted++;}
const tuples=[...map.values()].sort((a,b)=>`${a.vehicleName} ${a.modelYear||0} ${a.engineCc||0}`.localeCompare(`${b.vehicleName} ${b.modelYear||0} ${b.engineCc||0}`,"ko"));
...
const manifest={schemaVersion:1,id:"korea-jeju-registration-file",authority:"government_registration",fetchedAt:new Date().toISOString(),status:"complete_compacted",sourceUrl:URL,raw:{bytes:raw.length,sha256:sha256(raw),encoding,rows:parsed.length,storedInGit:false,note:"Raw 424k registration rows are not committed because duplicate registrations add repository weight without adding vehicle knowledge; unique technical/name tuples plus observation counts are committed."},headers,columnIndexes:ix,counts:{rowsParsed:parsed.length,rowsIn2020Plus:accepted,uniqueTuples:tuples.length},files};
