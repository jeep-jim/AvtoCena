...
for(let i=0;i<selected.length;i++){
 ...
 try{
  ...
  const years=[...text.matchAll(/(?:平成\s*)?(20(?:1\d|2[0-6]))\s*年?/g)].map(m=>Number(m[1]));const maxYear=years.length?Math.max(...years):null;if(maxYear&&maxYear<2010){manifest.skippedOld++;continue;}
  const name=`${String(i+1).padStart(4,"0")}-${digest.slice(0,16)}.txt`;await fs.writeFile(path.join(OUT,name),text);manifest.files.push({file:`mlit/pdf-text/${name}`,sourceUrl:ref.url,title:ref.text||null,pdfBytes:pdf.length,pdfSha256:digest,textBytes:Buffer.byteLength(text),years:[...new Set(years)].sort()});manifest.converted++;
 }catch(error){manifest.failed++;manifest.status="partial";manifest.errors.push({url:ref.url,title:ref.text||null,error:String(error?.message||error)});}finally{clearTimeout(timer);if(tmpPdf)await fs.rm(tmpPdf,{force:true});if(tmpTxt)await fs.rm(tmpTxt,{force:true});}
