...
function uniqueImages(images) {
  ...
  for (const image of credibleCatalogImages(Array.isArray(images) ? images : [])) {
    const key = imageKey(image); if (!key || seen.has(key)) continue;
    seen.add(key); result.push(image); if (result.length >= maxImages) break;
  }
  return result;
}
...
async function runWithConcurrency(items, concurrency, worker) {
  ...
  await Promise.all(Array.from({ length: Math.min(Math.max(1, concurrency), items.length) }, async () => {
    while (true) { const current = cursor++; if (current >= items.length) return; results[current] = await worker(items[current], current); }
  }));
  return results;
}
...
function addOffer(offer, origin) {
  ...
  offers.set(offer.id, offer); sourceCounts.set(offer.sourceId, sourceCount(offer.sourceId) + 1);
  ...
  report.saved = offers.size; return true;
}
...
function writeProgress(stopReason = report.stopReason, partial = true) {
  ...
  writeChain = writeChain.catch(() => undefined).then(async () => { await fs.writeFile(temporary, JSON.stringify(snapshot, null, 2)); await fs.rename(temporary, outputFile); report.checkpoints++; });
  return writeChain;
}
let lastCheckpointSize = 0;
async function checkpointIfNeeded(force = false) { if (!force && offers.size - lastCheckpointSize < checkpointEvery) return; lastCheckpointSize = offers.size; await writeProgress(deadlineReached() ? "deadline" : "running", true); }
...
async function processSourcePage(state) {
  ...
  const rows = Array.isArray(fetched?.items) ? fetched.items : []; report.seen += rows.length;
  const remaining = Math.max(0, targetPerSource - sourceCount(state.sourceId)); const batchIds = new Set(); const normalizedRows = [];
  for (const raw of rows) {
    let base = null; try { base = state.source.normalizeOffer(raw); } catch { base = null; }
    if (!base || base.market !== market || base.sourceId !== state.sourceId || !base.id || offers.has(base.id) || batchIds.has(base.id)) continue;
    batchIds.add(base.id); normalizedRows.push(base); if (normalizedRows.length >= remaining) break;
  }
  ...
  for (const prepared of preparedRows) { if (!prepared) { report.rejected++; continue; } if (addOffer(prepared, "fresh_listing")) state.savedFresh++; }
  const after = sourceCount(state.sourceId); state.emptyPages = after === before ? state.emptyPages + 1 : 0;
  if (!sourceHasCapacity(state.sourceId)) { state.done = true; state.stopReason = "source_target_reached"; return; }
  if (state.emptyPages >= maxEmptyPages) { state.done = true; state.stopReason = "no_progress"; return; }
  state.cursor = fetched?.nextCursor || null;
  if ((fetched?.finished && !state.cursor) || !state.cursor) { state.done = true; state.stopReason = "source_finished"; }
}
...
for (let index = 0; index < restoredSeeds.length && !deadlineReached(); index += prepareConcurrency) {
  ...
  for (const prepared of preparedRows) { if (!prepared || !addOffer(prepared, "revalidated_listing")) continue; report.seedSaved++; }
