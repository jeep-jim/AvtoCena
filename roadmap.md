...
Обычный git push недоступен без авторизации CLI; подготовленные изменения
отправляются через подключённый GitHub в feat/catalog-full-specifications.
Запуск pilot подтверждается отдельно по GitHub Actions, не по наличию YAML.
