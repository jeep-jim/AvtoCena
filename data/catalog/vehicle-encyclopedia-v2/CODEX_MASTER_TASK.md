...
Work in repository `jeep-jim/AvtoCena` on branch `knowledge/vehicle-encyclopedia-v2-20260812`.

Read `data/catalog/vehicle-encyclopedia-v2/AGENTS.md` first and obey it as a hard boundary.

## Objective

Build a large, accurate, source-backed automotive encyclopedia for AvtoCena, covering passenger/light-passenger vehicle brands, models, generations and variants whose production overlaps 2015-present. Start with all canonical brands in `apps/web/lib/catalog/brands.ts`, then identify globally relevant passenger-car brands missing from that list and REPORT them rather than editing production brand code.
...
3. Implement a validator under `scripts/vehicle-encyclopedia/` for `schema/brand.schema.json` plus semantic checks:
...
Research a representative pilot set first: Toyota, BMW, Mercedes-Benz, Hyundai, Kia, Volkswagen, BYD, Geely, Honda, Nissan.

For each brand create `data/catalog/vehicle-encyclopedia-v2/brands/<brand-slug>.json` using the schema. Validate after every brand. Commit in small reviewable batches.
