...
## Coverage and business priority

The approved priority pass is deliberately demand-led:

- Japan: model years 2015 through 2026.
- Every other active AvtoCena market: model years 2020 through 2026.
- First: vehicles appearing in active market listings and vehicles whose names/specifications currently prevent a calculation.
- Next: mass-market passenger cars bought by ordinary customers.
- Later: low-volume, exotic and heritage vehicles for long-tail matching and SEO.

“Covered” means the full identity chain is source-backed: brand → canonical model → generation/facelift → market variant/trim → available technical specifications and aliases. A raw marketplace title or a legacy row is only a research candidate; it is not counted as finished encyclopedia coverage.

The final denominator is not guessed in advance. It is built from official model inventories plus normalized active-listing identities for each market, then completion is measured against that denominator. The generated `reports/brand-queue.json` separates current canonical records from legacy candidates and reports how many legacy variants lack usable production years.

Before model coverage can be published, the brand registry has its own hard gate. Every parser-observed make and every current official passenger/light-passenger brand must resolve to a source-backed English/Latin canonical identity. Every publishable brand must also own authentic, reviewed light- and dark-theme logos. Canonical staged logos are transparent PNGs on an exact 90 × 60 px canvas; text fallbacks and generated marks never count as complete.
...
- `assets/brand-logos/{dark,light}/` — isolated normalized 90 × 60 px staging assets; not read by production.
...
brand ──────────────→ light/dark brand-logo media
```
...
python3 scripts/vehicle-encyclopedia/normalize-brand-logos.py
node scripts/vehicle-encyclopedia/audit-brand-readiness.mjs
node scripts/vehicle-encyclopedia/audit-brand-denominator.mjs
