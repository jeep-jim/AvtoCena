...
| China | Che168 Global used | `autohome_used_china_open` | PASS / SOURCE EXHAUSTED | Dedicated API adapter uses `globalapi.che168.com`. Strict 10, 100 and 1000 passed. Source-exhaustion stage scanned 170 pages / 4,064 listing rows and accepted 3,258 exact unique cards; source then finished normally. | Treat ~3.2K as verified current strict inventory for this source. Daily refresh should use API pages/IDs incrementally rather than re-detailing the whole source. |
...
| China | Autohome new | `autohome_new_china_open` | PASS / CERTIFYING | Dedicated exact spec adapter now passes readiness and strict 10/100. It binds list price in CNY, exact spec/config fields and exact product images. UTF-8/GB decoding bug was fixed. | Strict 1000 is the next running certification stage; advance only on success. |
...
| Japan | Prestige Japan auctions | `prestige_japan_auctions_open` | CONTRACT DISCOVERY | Public `/auctions/` page works on GitHub runner (~293 KB) and exposes a real search form with `auction-date=Past`, maker/model/year/auction filters and `search_cars_action`. | Trace the exact AJAX/search handler, submit Past search, then verify final price and exact photos before writing a dedicated adapter. |
...
The live search exposed 4,064 rows over 170 pages during source-exhaustion certification. 3,258 unique rows survived the strict core/gallery gate. This is a legitimate `up to 30K` result; never duplicate rows to manufacture 30,000.
...
The public search page is accessible on GitHub-hosted runners and exposes a real `jas-search-form`. Search controls include Today/Future/Past plus year, make, model, auction date, chassis, transmission, auction name, grade and color. This is currently the strongest public candidate for the user's required historical Japanese results. The next gate is the exact XHR/AJAX contract and then per-card proof of `Final Price` and photo identity.
