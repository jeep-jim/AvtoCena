...
| China | Autohome new | `autohome_new_china_open` | PASS 1000 / 30K STAGE RUNNING | Dedicated exact spec adapter passed strict 10, 100 and 1000. The 1000 run scanned 9 list pages, saw 1,200 rows and returned 1,000 unique strict cards; source reported 1,002 saved candidates before output truncation to target. | Run the final `up to 30K` stage to either 30,000 exact cards or verified source exhaustion; no publication during certification. |
...
| Japan | Prestige Japan auctions | `prestige_japan_auctions_open` | SEARCH CONTRACT VERIFIED / RESULT PRICE PROBING | Exact AJAX handler `search_results_car_dev` works with `auction-date=Past`, returns real per-vehicle `auction-vehicle-display/?car_id=...` links and large historical result sets. Sampled USS details expose exact vehicle data and many images, but `Final Price` is `Not yet available` and the source itself notes USS stopped publishing sale prices online. | Probe non-USS auction filters for any exact published final-price/status contract. Do not use start price as sale result. |
...
Dedicated adapter reads the current list under `car.autohome.com.cn/price/...`, binds a numeric `specId`, converts the displayed `万` list price to exact CNY, opens `www.autohome.com.cn/spec/<specId>/` and `car.autohome.com.cn/config/spec/<specId>.html`, and preserves exact config values. Exact product images are restricted to the Autohome product-image CDN path. Maximum motor/system power published by Autohome is retained as raw source data for electrified models and is not re-labelled as customs 30-minute power.

Strict scale result so far: 1000 unique cards passed. That run required 9 list pages, saw 1,200 rows, and rejected 137 candidates for insufficient exact gallery while preserving 1,000 strict output cards. The final high-volume stage is intentionally non-publishing and succeeds only on 30,000 exact cards or verified source exhaustion.
...
Both generic past-auction wrapper layers now require an explicit `auctionResultPriceVerified`/`resultPriceVerified` or `auctionPriceKind=published_result` marker before setting `sold` / `auction_result`. A merely positive parsed price is no longer enough.

### Japan: Prestige Motorsport

The public `/auctions/` page exposes a real `jas-search-form`. The verified AJAX flow is: resolve make/model, then call `search_results_car_dev` with `auction-date=Past`. It returns real historical search HTML and exact `auction-vehicle-display/?car_id=...` records. A Toyota Alphard probe returned a historical result total above 33,000 and exact vehicle pages with large image sets. Sampled USS pages explicitly reported `Final Price: Not yet available` and state that USS stopped publishing sale prices online after December 2016. A venue-filtered probe is used to determine whether non-USS auctions expose an exact result price before this source is allowed into `auction_result` publication.
