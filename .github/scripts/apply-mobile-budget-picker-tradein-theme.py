...
s = path.read_text()
start = s.index('function MobileBudgetPicker(')
end = s.index('\nfunction ElectricFilter(', start)
replacement = r'''function MobileBudgetPicker({ value, options, onChange }: { value: string; options: Option[]; onChange: (value: string) => void }) {
