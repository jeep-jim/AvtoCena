  ...
  async fetchPage(cursor?: string | null): Promise<CatalogFetchResult> {
    ...
    for (let index = 0; index < result.items.length; index += 8) {
      const batch = await Promise.all(result.items.slice(index, index + 8).map(async (raw) => {
        ...
        // The listing card is scoped to one vehicle and is therefore the safest price source.
        // Never replace it with a maximum number found across the whole detail page.
        if (currentPrice > 0 && ["PLN", "EUR"].includes(currentCurrency)) return raw;

        const url = String(row.url || "");
        if (!url) return raw;
        const markup = await fetchMarkup(url).catch(() => "");
        if (!markup) return raw;
        const parsed = embeddedPrice(markup);
