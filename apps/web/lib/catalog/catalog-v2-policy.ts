...
function hasSourceCalculationInputs(offer: Partial<VehicleOffer>) { return Boolean(number(offer.sourcePrice) && String(offer.sourceCurrency || "").trim() && number(offer.year) && number(offer.powerHp)); }
...
export function classifyCatalogV2Offer(offer: Partial<VehicleOffer>, options: CatalogV2PolicyOptions = CATALOG_V2_DEFAULT_POLICY): CatalogV2Classification {
  ...
  if (offer.market === "japan") {
    ...
    if (!number(offer.sourcePrice)) return { tier: "rejected", eligible: false, reason: "japan_final_price_missing", totalRub, ageYears, powerHp, popularityDecile: popularity };
