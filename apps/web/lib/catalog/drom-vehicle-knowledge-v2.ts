...
export function dromDocumentToLines(input: string) {
  return decodeHtml(input)
    .replace(/<script[\s\S]*?<\/script>/gi, " ")
    .replace(/<style[\s\S]*?<\/style>/gi, " ")
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<\/(?:td|th)>/gi, " | ")
    .replace(/<\/(?:tr|h[1-6]|p|li|section|article|div)>/gi, "\n")
    .replace(/<[^>]+>/g, " ")
    .split(/\n+/)
    .map((line) => line
      .replace(/^\s*#{1,6}\s*/, "")
...
function periodFromText(value: string) {
  ...
  const productionFrom = monthDate(match[1]);
  const productionTo = /н\.?\s*в\.?/i.test(match[2]) ? undefined : monthDate(match[2], true);
  return {
    productionFrom,
    productionTo,
    yearFrom: productionFrom ? Number(productionFrom.slice(0, 4)) : undefined,
    yearTo: productionTo ? Number(productionTo.slice(0, 4)) : undefined,
  };
}
...
function generation(value: string) {
  ...
  if (!match) return undefined;
  return `${match[1]} поколение${/рестайлинг/i.test(value) ? ", рестайлинг" : ""}`;
}

function engineCode(value: string) {
  const columns = value.split(/\||\t/).map((part) => part.trim()).filter(Boolean);
  const candidates = columns.slice(1).flatMap((column) => column.match(/\b[A-Z0-9][A-Z0-9-]{1,12}\b/g) || []);
  return candidates.find((candidate) => !/^(CVT|DCT|AWD|FWD|RWD|FF|FR|SUV|4WD)$/i.test(candidate) && !/^\d{3,4}$/.test(candidate));
}

function parseModification(line: string) {
  const hp = positiveNumber(line.match(/(\d+(?:[.,]\d+)?)\s*л\.?\s*с\.?/i)?.[1]);
  const liters = positiveNumber(line.match(/(\d+(?:[.,]\d+)?)\s*л(?:\.|,|\s)/i)?.[1]);
  const detectedFuel = fuel(line);
  if (!hp || !liters || !detectedFuel) return null;
...
export function looksLikeDromSpecsDocument(input: string) {
  const text = dromDocumentToLines(input).join("\n");
  return /(?:Объем|Объём) двигателя/i.test(text)
    && /Модификации/i.test(text)
    && /л\.?\s*с\.?/i.test(text);
}

export function parseDromVehicleVariantsV2(
  input: string,
  model: VehicleKnowledgeModel,
  sourceUrl: string,
  verifiedAt = new Date().toISOString(),
): Array<Omit<VehicleKnowledgeVariant, "id"> & { engineCode?: string }> {
  const lines = dromDocumentToLines(input);
  const variants: Array<Omit<VehicleKnowledgeVariant, "id"> & { engineCode?: string }> = [];
  let section = "";
  let period: ReturnType<typeof periodFromText> = null;

  for (let index = 0; index < lines.length; index++) {
    const line = lines[index];
    if (/^Двигател[ьь]\s+/i.test(line) && !/^Двигатель\s+[^,]+\s+технические/i.test(line)) {
      section = line;
    ...
    const candidates = [line, `${line} | ${lines[index + 1] || ""}`, `${line} | ${lines[index + 1] || ""} | ${lines[index + 2] || ""}`];
    const modification = candidates.map(parseModification).find(Boolean);
  ...
  for (const variant of variants) {
    if (variant.yearTo && variant.yearTo < 2011) continue;
    const key = [variant.generation || "", variant.productionFrom || "", variant.productionTo || "", variant.engineCc || "", variant.powerHp, variant.fuel || "", variant.transmission || "", variant.drive || "", variant.engineCode || ""].join("|");
