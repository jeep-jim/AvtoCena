...
export async function importCatalog(sourceIds?: string[]) {
  ...
  try {
    for (const source of catalogSources.filter(s => !sourceIds?.length || sourceIds.includes(s.sourceId))) {
      ...
      let cursor: string | null | undefined = policy.lastSuccessfulCursor || null; let page = 0; let sourceDetails = 0; const seen = new Set<string>(); let sourceOk = false; let sourceExhausted = false; let stoppedByLimit = false; let lastHealth: any = { ok: true, message: "ok", checkedAt: new Date().toISOString() };
      try {
