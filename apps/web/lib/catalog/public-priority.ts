...
export function catalogOfferVisibleRub(offer: Partial<VehicleOffer> | any) {
  return completeCalculation(offer)
    ? Math.round(positive(offer?.totalRub, 1_000_000_000))
    : 0;
}
...
export function catalogPublicPriority(offer: Partial<VehicleOffer> | any): CatalogPublicPriority {
  ...
  const visibleRub = calculated ? Math.round(positive(offer?.totalRub, 1_000_000_000)) : 0;
  ...
  if (!japanAuction && visibleRub > maximumRub) return { eligible: false, tier: 99, reason: "above_public_price_limit", visibleRub, ageYears, powerHp, popularityDecile, calculated, imageCount, japanAuction };
