...
export function credibleCatalogImages(images: CatalogImage[]) {
  ...
  for (const image of images || []) {
    if (!image?.url || Number(image.size || 0) < 10000) continue;
...
export function isCrediblePublicOffer(offer: VehicleOffer) {
  ...
  if (!Number.isFinite(totalRub) || totalRub < 250000 || totalRub > 150000000) return false;
  if (year >= currentYear - 6 && totalRub < 650000) return false;
  if (PREMIUM_MAKES.test(clean(offer.make)) && year >= 2015 && totalRub < 1000000) return false;

  const images = credibleCatalogImages(offer.images || []);
  const sourceId = offer.sourceId || "";
  const isAuction = offer.offerType === "auction" || sourceId.includes("jpauc");
  const strictSource = /(?:_open|che168|autohome|tcv|japan_partner|gaspedaal|otomoto|standvirtual|autoscout|mobile_de)/i.test(sourceId);
  if (!images.length) return false;
  if (!isAuction && strictSource && offer.market !== "uae" && images.length < 2) return false;
