...
const FINANCING_ONLY_PRICE_RE = /\b(?:aed|usd|eur)\s*\d[\d,\s]*(?:\/\s*month|p\.?\s*m\.?|per\s+month|monthly)(?=\s|$|[•·|,-])/i;
...
export function isFinancingOnlyListingText(value: unknown) {
  return FINANCING_ONLY_PRICE_RE.test(clean(value));
}
function meaningfulTitle(value: unknown) {
  const text = clean(value);
  return text.length >= 2 && text.length <= 180 && /[\p{L}\p{N}]/u.test(text) && !isFinancingOnlyListingText(text) && !GENERIC_LISTING_RE.test(text) && !NON_VEHICLE_RE.test(text);
}
