...
export type SourceConfig = { id:string; market:MarketId; enabled:boolean; mode:SourceMode; lastSuccessAt?:string|null; lastErrorAt?:string|null; lastError?:string|null; requestDelayMs:number; allowed:boolean; notes:string };
...
export class ManualJsonAdapter implements SourceAdapter { constructor(public config: SourceConfig){} async fetchListings(){return []} async fetchListingDetails(l:unknown){return l} normalizeListing(l:unknown){return l as VehicleOffer} extractImages(l:unknown){return Array.isArray((l as VehicleOffer).images)?(l as VehicleOffer).images:[]} async healthCheck(){return {ok:this.config.allowed,message:this.config.notes,checkedAt:new Date().toISOString()}} }
