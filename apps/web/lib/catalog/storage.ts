...
export function buildFeaturedOffers(limit = 24) { return readAllMarketOffers().filter((offer)=>offer.availability === "live" && offer.importEligibility !== "blocked").slice(0, limit).map((offer)=>({ id: offer.id, market: offer.market, brand: offer.brand, model: offer.model, trim: offer.trim, year: offer.advertisedModelYear || offer.year, mileageKm: offer.mileageKm, bodyType: offer.bodyType, engineCc: offer.engineCc, advertisedPower: offer.advertisedPower, sourceName: offer.sourceName, sourceUrl: offer.sourceUrl, sourcePriceLocal: offer.priceLocal, currency: offer.currency, calculationStatus: offer.calculationStatus, calculationComplete: offer.calculationComplete, knownCostRub: offer.calculationSnapshot?.sourcePriceRub || null, lastCheckedAt: offer.lastCheckedAt, imageMode: offer.imageMode })); }
