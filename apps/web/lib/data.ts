...
/** Read only the newest chunks for a live feed; never load its complete history. */
export async function readRecentChunkedDataJson<T>(relativePath:string,limit=100,accept:(row:T)=>boolean=()=>true):Promise<T[]> {
 const paths=collectionPaths(relativePath),storage=getJsonStorage();
 const stored=await storage.readJsonWithMeta<unknown>(paths.rel(paths.indexFile),null);
 if(!validIndex(stored.value))return (await readStorageOrSeed<T[]>(relativePath,[])).filter(accept).slice(0,limit);
 const rows:T[]=[];
 for(const chunk of [...stored.value.chunks].reverse()) {
  rows.push(...(await storage.readJson<T[]>(paths.rel(chunk.file),[])).filter(accept));
