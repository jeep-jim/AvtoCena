...
export function PublicHeader({ backHref, backLabel = "Назад", className = "" }: Props) {
  ...
  const autocatalogActive = pathname === "/cars/autocatalog" || pathname.startsWith("/cars/brand/");
  const catalogActive = (pathname === "/cars" || pathname.startsWith("/cars/")) && !autocatalogActive;
        ...
        <div className="mx-auto flex h-16 w-full max-w-[1500px] items-center justify-between gap-3 px-4 md:px-8">
          ...
          <div className="flex items-center gap-1.5 md:gap-2">
            ...
            <nav className="hidden items-center gap-1 text-sm font-black text-white/72 md:flex" aria-label="Основная навигация">
              ...
              <Link href="/cars/autocatalog" className={`ac-catalog-nav flex h-11 items-center rounded-xl px-4 transition ${autocatalogActive ? "is-active" : ""}`} aria-current={autocatalogActive ? "page" : undefined}>
