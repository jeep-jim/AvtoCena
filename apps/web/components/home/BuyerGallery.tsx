...
export function BuyerGallery({ images }: { images: string[] }) {
  ...
  return <section className="mt-8 overflow-hidden">
    ...
    <div ref={rail} onPointerEnter={() => { paused.current = true; }} onPointerLeave={() => { paused.current = false; }} onPointerDown={() => { paused.current = true; }} onPointerUp={() => { paused.current = false; }} onTouchStart={() => { paused.current = true; }} onTouchEnd={() => { paused.current = false; }} className="ac-buyers-rail ac-hide-scrollbar mt-4 flex gap-3 overflow-x-auto pb-2">
