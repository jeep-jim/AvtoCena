...
export function OfferLeadForm({ offerId }: Props) {
  ...
  async function submit(e: React.FormEvent) { e.preventDefault(); setStatus("sending"); setMessage(""); const operationId = getOperationId(); try { const res = await fetch("/api/leads", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ operationId, offerId, name, phone, source: "catalog_offer", attribution: captureAttributionFromBrowser() }) }); const json = await res.json(); if (!res.ok || !json.ok) throw new Error(json.error || "Не удалось отправить заявку"); sessionStorage.removeItem(storageKey); setStatus("success"); setMessage("Заявка отправлена. Менеджер проверит наличие и точную стоимость автомобиля."); setName(""); setPhone(""); } catch (error: any) { setStatus("error"); setMessage(error?.message || "Ошибка сети. Повторная отправка использует тот же номер операции и не создаст дубль."); } }
