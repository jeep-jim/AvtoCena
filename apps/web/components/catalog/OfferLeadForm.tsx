...
export function OfferLeadForm({ offerId }: Props) {
  ...
  return <form onSubmit={submit} className="mt-6 grid gap-3">
    <div className="mb-1"><div className="text-xs font-black uppercase tracking-[0.16em] text-red-500">Запросить точный расчёт</div><p className="mt-1 text-sm font-medium leading-6 text-white/48">Оставьте контакты — менеджер подтвердит наличие и итоговую цену.</p></div>
    <label><span className="mb-1.5 block text-[11px] font-black uppercase tracking-[0.13em] text-white/55">Имя</span><input value={name} onChange={(e)=>setName(e.target.value)} name="name" placeholder="Как к вам обращаться" className="soft-input w-full rounded-2xl px-4 py-3.5" /></label>
    <label><span className="mb-1.5 block text-[11px] font-black uppercase tracking-[0.13em] text-white/55">Телефон</span><input value={phone} onChange={(e)=>setPhone(e.target.value)} name="phone" placeholder="Номер для связи" className="soft-input w-full rounded-2xl px-4 py-3.5" /></label>
    <button disabled={status === "sending"} className="avto-button mt-1 rounded-2xl px-5 py-4 font-black disabled:opacity-60">{status === "sending" ? "Отправляем…" : "Оставить заявку"}</button>
    {message ? <div className="rounded-2xl bg-red-500/10 p-3 text-sm font-bold text-red-100">{message}</div> : null}
    <button type="button" onClick={shareLink} className="flex items-center justify-center gap-2 rounded-2xl bg-white/[0.055] px-5 py-4 font-black text-white/78 transition hover:bg-white/[0.09]"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M4 12V20H20V12M12 16V3M7 8L12 3L17 8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>{copyStatus === "success" ? "Ссылка отправлена" : copyStatus === "error" ? "Не удалось отправить" : "Отправить ссылку"}</button>
  </form>;
}
