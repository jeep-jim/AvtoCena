...
export function JapanSectionTabs({active,params={}}:{active:"auction"|"green";params?:Record<string,string|undefined>}) {
 ...
 return <nav aria-label="Разделы Японии" className="ac-japan-sections my-5 flex flex-wrap gap-2.5 text-sm font-bold">
  <Link href={target(false)} aria-current={active==="auction"?"page":undefined} className="ac-auction-button">Аукционная Япония</Link>
  <Link href={target(true)} aria-current={active==="green"?"page":undefined} className="ac-green-button">Зелёный угол · в наличии</Link>
 </nav>;
}
