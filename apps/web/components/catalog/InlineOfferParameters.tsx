...
export function InlineOfferParameters({offerId,initial,price,children,priceBadges,exportWarning,reportedVolume,showCommercial=false,isPickup=false,researchContext="",autoCalculate=false,sourcePriceOnly=false}:{offerId:string;reportedVolume?:number;autoCalculate?:boolean;sourcePriceOnly?:boolean;initial:ParameterDraft;price:ReactNode;children:ReactNode;priceBadges?:ReactNode;exportWarning?:string;showCommercial?:boolean;isPickup?:boolean;researchContext?:string}) {
 ...
 return <div className={`ac-inline-parameters ${showCalculation?"ac-personal-parameters":""}`}>
  ...
  {result?.breakdown?.length?<details className="ac-offer-breakdown mt-4 rounded-2xl bg-[var(--ac-surface-2)] p-4"><summary className="cursor-pointer pr-4 font-bold">{dirty?"Структура расчёта по вашим параметрам":"Структура цены"}</summary><dl className="mt-3 space-y-2 text-xs">{result.breakdown.map((row,i)=>{const note=row.id === "export" ? "Фиксированная сумма из настроек рынка на услуги в стране покупки. Состав услуг требует подтверждения; это не надбавка за санкционные ограничения." : visibleBreakdownNote(row.note);return <div key={`${row.id}-${i}`} ><div className="flex justify-between gap-3"><dt>{row.label||row.title||row.id}{note ? <p className="mt-1 text-[11px] font-normal text-[var(--ac-muted)]">{note}</p> : null}{/utilization|утил/i.test(`${row.id} ${row.title||row.label||""}`) ? <RecyclingFeeHelp info={powerInfo} /> : null}</dt><dd className="shrink-0 whitespace-nowrap">{Math.round(row.amountRub).toLocaleString("ru-RU")} ₽</dd></div>{i === 0 ? <ContractPaymentSummary plan={result.paymentPlan} embedded /> : null}</div>})}</dl></details>:null}
  {children}
