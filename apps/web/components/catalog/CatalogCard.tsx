...
export function CatalogCard({ offer, compact = false }: { offer: any; compact?: boolean }) {
  ...
  return (
    <article className="group relative min-w-0 overflow-hidden rounded-[1.55rem] border border-white/9 bg-white/[0.045] shadow-[0_18px_70px_rgba(0,0,0,.22)] transition hover:-translate-y-1 hover:border-red-300/25 hover:bg-white/[0.06]">
      <Link href={href} className="block">
        <div className={`relative overflow-hidden bg-white/[0.04] ${compact ? "h-48" : "h-56"}`}>
          {imageUrl ? (
            <img src={imageUrl} alt={o.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.025]" />
          ) : (
            <div className="flex h-full items-center justify-center text-sm font-black text-white/35">Фото загружается</div>
          )}
          <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/86 to-transparent" />
          <div className="absolute left-3 top-3 rounded-full border border-white/18 bg-black/38 px-3 py-1 text-[11px] font-black uppercase tracking-[0.11em] text-white/90 backdrop-blur">
            {o.marketLabel}
          </div>
          <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between gap-3">
            ...
            <span className="shrink-0 rounded-full bg-black/45 px-3 py-1 text-xs font-black text-white/88 backdrop-blur">{o.year}</span>
          </div>
        </div>
