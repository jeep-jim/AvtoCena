      ...
      <Link href={href} className="block">
        <div className={`relative overflow-hidden bg-white/[0.04] ${mediaHeight}`}>
          ...
          <div className={`ac-on-image absolute flex items-end justify-between text-white ${dense ? "bottom-2 left-2 right-2 gap-1.5 sm:bottom-3 sm:left-3 sm:right-3 sm:gap-3" : "bottom-3 left-3 right-3 gap-3"}`}>
            <div className={`line-clamp-2 min-w-0 font-black leading-[1.04] tracking-[-0.03em] text-white drop-shadow-[0_2px_15px_rgba(0,0,0,.7)] ${dense ? "text-[13px] sm:text-[17px] sm:leading-[1.08]" : "text-[17px] leading-[1.08]"}`}>{o.title}</div>
