...
export function CatalogCard({ offer, compact = false, dense = false }: { offer: any; compact?: boolean; dense?: boolean }) {
  ...
  const snapshotRub = exactTotalRub || sourcePriceRub(o);
  const snapshot = {
    id: o.id, title: o.title, price: snapshotRub || null, totalRub: snapshotRub || null, previousTotalRub: o.previousTotalRub,
  ...
  const priceLabel = !exactTotalRub ? `${yearLabel} · расчёт уточняется` : estimated ? `${yearLabel} · ориентир` : yearLabel;
      ...
      <Link href={href} className="block overflow-hidden rounded-[1.35rem]">
        ...
        <div className={dense ? "p-2.5 sm:p-3.5" : "p-3.5"}>
          {isAuctionResult && hasSourcePrice ? <div>
            ...
            {exactTotalRub > 0 ? <div className={`${dense ? "mt-1 text-[9px] sm:text-[11px]" : "mt-1.5 text-[11px]"} font-bold text-white/52`}>Расчёт под ключ: {new Intl.NumberFormat("ru-RU").format(Math.round(exactTotalRub))} ₽</div> : <div className={`${dense ? "mt-1 text-[9px] sm:text-[11px]" : "mt-1.5 text-[11px]"} font-bold text-white/52`}>Расчёт таможни и утиля уточняется</div>}
          </div> : !exactTotalRub && hasSourcePrice ? <div>
