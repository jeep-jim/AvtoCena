...
export function AuctionCardPrice({ offer, label, dense = false, interactiveGrade = false, priceClassName = "text-[22px]" }: AuctionCardPriceProps) {
  ...
  return <div className={`ac-auction-card-price relative min-w-0 ${interactiveGrade ? "grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3" : ""}`}>
    <div className={`${dense ? "text-[8px] sm:text-[10px]" : "text-[10px]"} ${interactiveGrade ? "col-start-1 row-start-1" : ""} ac-price-trend-label min-w-0 font-black uppercase tracking-[0.19em] text-[var(--ac-text)]`}>{label}</div>
    ...
    {interactiveGrade ? <div className="col-start-2 row-start-1 row-span-2 flex items-center justify-center"><JapanAuctionBadges offer={offer} dense={dense} interactive /></div> : null}
  </div>;
}
