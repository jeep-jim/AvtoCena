...
export function CatalogCover({src, alt, eager}: {src:string; alt:string; eager:boolean}) {
  ...
  return <img src={stage === 1 ? src : thumbnail} alt={alt} width={400} height={300} className="h-full w-full object-cover object-[center_42%]" loading={eager ? "eager" : "lazy"} decoding="async" fetchPriority={eager ? "auto" : "low"} onError={event => { if (!retryProtectedPhoto(event)) setFailure({src,stage:stage === 0 && thumbnail !== src ? 1 : 2}); }} />;
}
