...
const OfferPdfPreview = dynamic(() => import("./OfferPdfPreview"), { ssr: false });
...
export function OfferPdfButton({offerId,draft}:{offerId:string;draft:Record<string,string>}) {
 ...
 async function generate(mode:"view"|"download") {
  ...
  try{
   ...
   else {const url=URL.createObjectURL(blob);urls.current.push(url);const link=document.createElement("a");link.href=url;link.download=`АвтоЦена-${offerId.replace(/[^a-zA-Z0-9_-]/g,"_")}.pdf`;document.body.appendChild(link);link.click();link.remove();}
   dialog.current?.close();
  }catch(e){setError(e instanceof Error?e.message:"Не удалось подготовить PDF");}
 ...
 return <>
  ...
  {preview ? <OfferPdfPreview blob={preview} filename={`АвтоЦена-${offerId.replace(/[^a-zA-Z0-9_-]/g,"_")}.pdf`} onClose={()=>setPreview(null)}/> : null}
 </>;
}
