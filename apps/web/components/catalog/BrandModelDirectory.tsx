...
function modelKnowledgeLabel(model: BrandModelLink) {
  ...
  if (model.knowledge?.trustedVariants) return `${model.knowledge.trustedVariants} модификаций`;
  return model.knowledge?.records ? `${model.knowledge.records} вариантов` : "";
}
  ...
  return <section className="mt-7 rounded-[1.8rem] bg-[var(--ac-surface)] p-5 md:p-7">
    <div className="flex items-center justify-between gap-3">
      <div>
        ...
        <p className="mt-1 text-xs font-bold text-[var(--ac-muted)]">Найдите модель, откройте характеристики и посмотрите автомобили со всех доступных рынков.</p>
      </div>
      {canExpand ? <button
        type="button"
      ...
      {filtered.map((model) => {
        ...
        >
          ...
          {open ? <div id={detailId} className="border-t border-black/5 px-3 pb-3 pt-3 dark:border-white/5 md:px-4 md:pb-4">
            ...
            </div> : <p className="rounded-xl bg-[var(--ac-surface)] px-3 py-3 text-xs font-bold leading-5 text-[var(--ac-muted)]">Характеристики этой модели дополняются.</p>}
