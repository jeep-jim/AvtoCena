...
export function CatalogFilters({ initial, facets }: { initial: Record<string, string>; facets?: Facets }) {
  ...
  const makeOptions = useMemo<Option[]>(() => { const values = facets?.makes?.length ? facets.makes : fallbackMakes; return [{ value: "", label: "Любая марка" }, ...[...new Set(values.map(clean).filter(Boolean))].sort((a,b) => label(a).localeCompare(label(b), "ru")).map((value) => ({ value, label: label(value) }))]; }, [facets]);
  ...
  return <><form className="ac-catalog-filter-panel ac-filter-panel mt-6 rounded-[1.8rem] p-3 md:p-4"><div className={`hidden gap-3 lg:grid ${expanded ? "lg:grid-cols-3" : "lg:grid-cols-[1fr_1fr_1fr_auto]"}`}><SearchSelect name="make" value={make} placeholder="Любая марка" searchPlaceholder="Найти марку" options={makeOptions} onChange={setMake} /><SearchSelect key={`desktop-${make}`} name="model" value={initial.make === make ? initial.model || "" : ""} placeholder="Любая модель" searchPlaceholder="Найти модель" options={modelOptions} /><SimpleSelect name="market" value={initial.market || ""} placeholder="Все рынки" options={markets} />{!expanded ? <button className="avto-button h-14 rounded-2xl px-12 text-base font-black">Найти</button> : null}</div><button type="button" onClick={() => setExpanded((current) => !current)} className="ac-advanced-toggle mx-auto mt-4 hidden min-h-10 items-center gap-2 rounded-xl px-4 text-sm font-black transition lg:flex">{expanded ? "Скрыть расширенный поиск" : "Расширенный поиск"}<Chevron open={expanded} /></button>{expanded ? <div className="ac-advanced-fields mt-4 hidden rounded-[1.35rem] p-4 lg:block"><AdvancedFields initial={initial} make={make} makeOptions={makeOptions} modelOptions={modelOptions} setMake={setMake} includePrimary={false} /><button className="avto-button mt-4 h-14 w-full rounded-2xl text-base font-black">Найти</button></div> : null}<div className="grid gap-3 lg:hidden"><SearchSelect name="make" value={make} placeholder="Любая марка" searchPlaceholder="Найти марку" options={makeOptions} onChange={setMake} /><SearchSelect key={`mobile-${make}`} name="model" value={initial.make === make ? initial.model || "" : ""} placeholder="Любая модель" searchPlaceholder="Найти модель" options={modelOptions} /><SimpleSelect name="market" value={initial.market || ""} placeholder="Все рынки" options={markets} /><div className="grid grid-cols-[minmax(0,1fr)_58px] overflow-hidden rounded-2xl"><button className="avto-button h-14 rounded-none text-base font-black">Найти</button><button type="button" onClick={() => setMobileOpen(true)} className="ac-filter-more-button flex h-14 items-center justify-center" aria-label="Открыть остальные фильтры"><SlidersIcon /></button></div></div></form>{mobileOpen ? <div className="fixed inset-0 z-[9998] bg-black/70 lg:hidden" onClick={() => setMobileOpen(false)}><form className="ac-catalog-filter-drawer ac-hide-scrollbar absolute inset-y-0 right-0 w-[min(92vw,390px)] overflow-y-auto p-5 pb-8" onClick={(event) => event.stopPropagation()}><div className="mb-6 flex items-center justify-between"><h2 className="text-2xl font-black">Фильтры</h2><button type="button" onClick={() => setMobileOpen(false)} className="ac-filter-close flex h-10 w-10 items-center justify-center rounded-xl text-2xl" aria-label="Закрыть">×</button></div><AdvancedFields initial={initial} make={make} makeOptions={makeOptions} modelOptions={modelOptions} setMake={setMake} /><button className="avto-button mt-8 h-14 w-full rounded-2xl text-base font-black">Показать результаты</button></form></div> : null}</>;
}
