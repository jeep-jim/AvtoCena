...
function AdvancedFields({ initial, make, setMake, includePrimary = true }: { initial: Record<string, string>; make: string; setMake: (value: string) => void; includePrimary?: boolean }) {
  ...
  return <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
    {includePrimary ? <>
      ...
      <SearchSelect key={make} name="model" value={initial.model || ""} placeholder="Любая модель" searchPlaceholder="Найти модель" options={modelOptions} />
  ...
  return <>
    <form className="ac-catalog-filter-panel ac-filter-panel mt-6 rounded-[1.8rem] p-3 md:p-4">
      <div className={`hidden gap-3 lg:grid ${expanded ? "lg:grid-cols-3" : "lg:grid-cols-[1fr_1fr_1fr_auto]"}`}>
        <SearchSelect name="make" value={make} placeholder="Любая марка" searchPlaceholder="Найти марку" options={makeOptions} onChange={setMake} />
        <SearchSelect key={`desktop-${make}`} name="model" value={initial.model || ""} placeholder="Любая модель" searchPlaceholder="Найти модель" options={modelOptions} />
        <SimpleSelect name="market" value={initial.market || ""} placeholder="Все рынки" options={markets} />
      ...
      {expanded ? <div className="ac-advanced-fields mt-4 hidden rounded-[1.35rem] p-4 lg:block">
        <AdvancedFields initial={initial} make={make} setMake={setMake} includePrimary={false} />
        <button className="avto-button mt-4 h-14 w-full rounded-2xl text-base font-black">Найти</button>
      </div> : null}

      <div className="grid gap-3 lg:hidden">
        <SearchSelect name="make" value={make} placeholder="Любая марка" searchPlaceholder="Найти марку" options={makeOptions} onChange={setMake} />
        <SearchSelect key={`mobile-${make}`} name="model" value={initial.model || ""} placeholder="Любая модель" searchPlaceholder="Найти модель" options={modelOptions} />
        <SimpleSelect name="market" value={initial.market || ""} placeholder="Все рынки" options={markets} />
