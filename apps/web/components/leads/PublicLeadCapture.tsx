...
function operationIdFor(request: LeadRequest, selectedIds: string[]) {
  const scope = request.mode === "offer"
    ? request.offerId
    : request.mode === "favorites"
      ? selectedIds.slice().sort().join("_") || "none"
      : request.source;
...
function GenericLeadBanner({ kind, onOpen }: { kind: HostKind; onOpen: () => void }) {
  ...
  return (
    <section className="ac-lead-capture-banner mt-8 grid gap-4 rounded-[1.7rem] bg-[var(--ac-surface)] p-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center md:p-6">
      ...
      <button type="button" onClick={onOpen} className="avto-button min-h-[52px] rounded-2xl px-6 py-3.5 text-sm font-black text-white md:min-w-48 md:text-base">
        Оставить заявку
      </button>
    </section>
  );
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`grid grid-cols-[54px_minmax(0,1fr)_26px] items-center gap-3 rounded-2xl p-2.5 text-left transition ${selected ? "bg-red-500/10" : "bg-[var(--ac-surface-2)]"} disabled:opacity-45`}
              aria-pressed={selected}
            >
              <div className="h-[54px] w-[54px] overflow-hidden rounded-xl bg-[var(--ac-surface-3)]">
                {item.imageUrl ? <img src={item.imageUrl} alt="" className="h-full w-full object-cover" /> : null}
              </div>
              <div className="min-w-0">
                <div className="truncate text-sm font-black text-[var(--ac-text)]">{cleanText(item.title) || "Автомобиль"}</div>
                <div className="mt-1 flex flex-wrap gap-x-2 text-xs font-bold text-[var(--ac-muted)]">
                  {item.year ? <span>{item.year}</span> : null}
                  {price > 0 ? <span>{formatRub(price)}</span> : null}
                </div>
              </div>
...
function LeadDialog({
  ...
  function validate() {
    ...
    if (isFavorites) {
      if (!selectedIds.length) return "Выберите хотя бы один автомобиль из Избранного.";
      if (contactPreference === "call" && !cleanText(form.phone)) return "Укажите телефон для звонка.";
      if (contactPreference === "message" && !cleanText(messengerContact)) return `Укажите контакт в ${messenger === "telegram" ? "Telegram" : "MAX"}.`;
    } else {
      if (!cleanText(form.phone)) return "Укажите телефон для связи.";
      if (!isOffer && !cleanText(form.car)) return "Укажите интересующий автомобиль.";
      if (!isOffer && budgetRub <= 0) return "Укажите бюджет.";
    }
  ...
  async function submit(event: React.FormEvent) {
    ...
    const carText = isFavorites
      ? selectedFavorites.map((item) => cleanText(item.title) || item.id).join("; ")
      : cleanText(form.car);
    const telegram = isFavorites && contactPreference === "message" && messenger === "telegram" ? cleanText(messengerContact) : "";
    ...
    try {
      const response = await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          ...
          phone: isFavorites && contactPreference === "message" ? "" : cleanText(form.phone),
          telegram,
          max,
          ...
          contactPreference: isFavorites ? contactPreference : "call",
          messenger: isFavorites && contactPreference === "message" ? messenger : "",
          selectedOfferIds: isFavorites ? selectedIds : [],
          selectedOffers: isFavorites ? selectedFavorites.slice(0, 5).map((item) => ({
            id: item.id,
            title: cleanText(item.title),
            totalRub: Number(item.totalRub ?? item.price ?? 0) || null,
            year: item.year || null,
            marketLabel: cleanText(item.marketLabel),
      ...
      setStatus("success");
      const action = isFavorites && contactPreference === "message" ? "напишет вам" : "перезвонит вам";
      setMessage(`Заявка принята. Менеджер ${action} в рабочее время: ${MOSCOW_HOURS} (${NOVOKUZNETSK_HOURS}).`);
    } catch (error) {
      setStatus("error");
  ...
  const intro = isFavorites
    ? "Выберите до 5 автомобилей — рассчитаем стоимость по каждому варианту и свяжемся удобным способом."
    : isOffer
      ? "Автомобиль уже выбран. Оставьте контакты — менеджер проверит актуальность и подтвердит точный расчёт."
      : "Опишите, какой автомобиль нужен. Если его нет на сайте, мы всё равно проверим доступные рынки и подготовим расчёт.";
  ...
  return createPortal(
    <div className="ac-lead-dialog-backdrop fixed inset-0 z-[20000] flex items-end justify-center bg-black/70 backdrop-blur-sm lg:items-center lg:p-6" role="dialog" aria-modal="true" aria-labelledby="ac-lead-dialog-title" onMouseDown={(event) => { if (event.target === event.currentTarget && status !== "sending") onClose(); }}>
      <section className="ac-lead-dialog ac-hide-scrollbar max-h-[94dvh] w-full overflow-y-auto rounded-t-[28px] bg-[var(--ac-surface)] px-4 pb-[calc(20px+env(safe-area-inset-bottom))] pt-3 text-[var(--ac-text)] lg:max-h-[90vh] lg:max-w-[760px] lg:rounded-[28px] lg:p-7">
        ...
        {status === "success" ? (
          ...
          <form onSubmit={submit} className="mt-6 grid gap-4">
            ...
            {isFavorites ? (
              <div className="rounded-2xl bg-[var(--ac-surface-2)] p-2">
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setContactPreference("call")} className={`min-h-11 rounded-xl px-3 text-sm font-black ${contactPreference === "call" ? "bg-red-500 text-white" : "text-[var(--ac-muted)]"}`}>Позвонить</button>
                  <button type="button" onClick={() => setContactPreference("message")} className={`min-h-11 rounded-xl px-3 text-sm font-black ${contactPreference === "message" ? "bg-red-500 text-white" : "text-[var(--ac-muted)]"}`}>Написать в чат</button>
                </div>
              </div>
            ) : null}

            {isFavorites && contactPreference === "message" ? (
              <div className="grid gap-3 md:grid-cols-[220px_minmax(0,1fr)]">
                <div>
                  <FieldLabel>Мессенджер</FieldLabel>
                  <div className="grid h-[52px] grid-cols-2 gap-1 rounded-2xl bg-[var(--ac-surface-2)] p-1">
                    <button type="button" onClick={() => setMessenger("telegram")} className={`rounded-xl text-sm font-black ${messenger === "telegram" ? "bg-[#229ED9] text-white" : "text-[var(--ac-muted)]"}`}>Telegram</button>
                    <button type="button" onClick={() => setMessenger("max")} className={`rounded-xl text-sm font-black ${messenger === "max" ? "bg-[#7B61FF] text-white" : "text-[var(--ac-muted)]"}`}>MAX</button>
                  </div>
                </div>
            ...
            {!isFavorites && !isOffer ? (
              <div className="grid gap-3 md:grid-cols-2">
                <label className="relative block min-w-0">
                  <FieldLabel>Интересующее авто</FieldLabel>
                  <input
                    value={form.car}
                    onChange={(event) => setField("car", event.target.value)}
                    onFocus={() => setCarFocused(true)}
                    onBlur={() => window.setTimeout(() => setCarFocused(false), 120)}
                    placeholder="Например, Toyota RAV4"
                    className="soft-input h-[52px] w-full rounded-2xl bg-[var(--ac-surface-2)] px-4 outline-none"
                    autoComplete="off"
                  />
                  {carFocused && suggestions.length ? <div className="absolute left-0 right-0 top-[calc(100%+7px)] z-30 max-h-56 overflow-y-auto rounded-2xl bg-[var(--ac-surface-3)] p-2 shadow-2xl">{suggestions.map((item, index) => {
                    const label = cleanText(item.label) || [item.make, item.model].map(cleanText).filter(Boolean).join(" ");
                    return <button key={item.id || `${label}-${index}`} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { setField("car", label); setSuggestions([]); setCarFocused(false); }} className="block min-h-10 w-full rounded-xl px-3 py-2 text-left text-sm font-bold text-[var(--ac-text)] hover:bg-white/[.07]">{label}</button>;
                  })}</div> : null}
                  <span className="mt-1.5 block text-[11px] font-semibold leading-4 text-[var(--ac-muted)]">Можно выбрать подсказку или написать любую марку и модель вручную.</span>
                </label>
                <label className="block min-w-0"><FieldLabel>Бюджет</FieldLabel><div className="relative"><input value={formatBudgetInput(form.budget)} onChange={(event) => setField("budget", event.target.value.replace(/\D/g, ""))} inputMode="numeric" placeholder="Например, 3 000 000" className="soft-input h-[52px] w-full rounded-2xl bg-[var(--ac-surface-2)] px-4 pr-10 outline-none" /><span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 font-black text-[var(--ac-muted)]">₽</span></div></label>
              </div>
            ) : null}
            ...
            {status === "error" && message ? <div className="rounded-2xl bg-red-500/10 p-3 text-sm font-bold leading-5 text-red-300">{message}</div> : null}

            <button type="submit" disabled={status === "sending"} className="avto-button min-h-14 rounded-2xl px-6 text-base font-black text-white disabled:opacity-55">{status === "sending" ? "Отправляем…" : "Отправить заявку"}</button>
          </form>
        )}
      </section>
      <style jsx global>{`
        ...
        .ac-lead-dialog { animation: acLeadSheetUp .2s ease-out both; overscroll-behavior: contain; }
        .ac-lead-favorites-list { scrollbar-width: thin; }
        .ac-lead-capture-banner { box-shadow: none !important; }
        html[data-theme="light"] .ac-lead-capture-banner,
        html[data-theme="light"] .ac-lead-dialog { color: var(--ac-text) !important; }
...
function FavoritesPinnedActions({ onLead }: { onLead: () => void }) {
  return (
    <div data-ac-favorites-bar className="fixed inset-x-0 bottom-0 z-[9000] border-t border-white/10 bg-[var(--ac-surface)] px-3 pb-[calc(10px+env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl md:left-1/2 md:right-auto md:bottom-5 md:w-[min(720px,calc(100vw-40px))] md:-translate-x-1/2 md:rounded-[1.6rem] md:border md:p-3">
      <div className="mx-auto grid max-w-3xl grid-cols-2 gap-2.5">
        <button type="button" data-offer-action="messenger" className="h-14 min-w-0 rounded-[1.25rem] bg-[#00A2E8] px-3 text-[13px] font-black leading-none text-white sm:text-sm">Чат в мессенджере</button>
        <button type="button" onClick={onLead} className="h-14 min-w-0 rounded-[1.25rem] bg-[#22B14C] px-3 text-[13px] font-black leading-none text-white sm:text-sm">Оставить заявку</button>
      </div>
    </div>
  );
}
...
export function PublicLeadCapture() {
  ...
  useEffect(() => {
    ...
    const mount = () => {
      ...
      const next = targets.flatMap(({ kind, parent }) => {
        if (!parent) return [];
        const node = document.createElement("div");
        node.dataset.acLeadHost = kind;
  ...
  useEffect(() => {
    ...
    const click = (event: MouseEvent) => {
      ...
      const button = target?.closest<HTMLAnchorElement>("main.ac-brand-catalog-page a[href='/#form']");
      if (!button) return;
      event.preventDefault();
      event.stopPropagation();
      const brand = cleanText(document.querySelector<HTMLElement>("main.ac-brand-catalog-page h1")?.textContent).replace(/\s+под ключ$/i, "");
      setRequest({ mode: "generic", source: "brand_lead_action", car: brand });
    };
  ...
  useEffect(() => {
    ...
    const click = (event: MouseEvent) => {
      const target = event.target as HTMLElement | null;
      const button = target?.closest<HTMLElement>("[data-offer-action='lead']");
      if (!button) return;
      event.preventDefault();
      event.stopPropagation();
      const heading = document.querySelector<HTMLElement>("main.ac-offer-page h1");
      setRequest({ mode: "offer", source: "catalog_offer_request", offerId, car: cleanText(heading?.textContent) });
    };
    document.addEventListener("click", click, true);
    return () => document.removeEventListener("click", click, true);
  }, [pathname]);
  ...
  return (
    <>
      {hosts.map((host) => createPortal(
        <GenericLeadBanner kind={host.kind} onOpen={() => { const brand = host.kind === "brand" ? cleanText(document.querySelector<HTMLElement>("main.ac-brand-catalog-page h1")?.textContent).replace(/\s+под ключ$/i, "") : ""; setRequest({ mode: "generic", source: `${host.kind}_lead_banner`, car: brand }); }} />,
        host.node,
      ))}
      {pathname === "/favorites" && favorites.length && !request ? <FavoritesPinnedActions onLead={() => setRequest({ mode: "favorites", source: "favorites_request" })} /> : null}
      {request ? <LeadDialog key={`${request.mode}:${request.mode === "offer" ? request.offerId : request.source}`} request={request} favorites={favorites} onClose={() => setRequest(null)} /> : null}
    </>
  );
}
