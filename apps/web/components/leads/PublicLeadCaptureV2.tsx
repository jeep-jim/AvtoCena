      ...
      <div className="relative w-full lg:max-w-[760px]" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <section ref={panelRef} className="ac-lead-dialog ac-hide-scrollbar max-h-[94dvh] w-full overflow-y-auto rounded-t-[28px] bg-[var(--ac-surface)] px-4 pb-[calc(20px+env(safe-area-inset-bottom))] pt-7 text-[var(--ac-text)] lg:max-h-[90vh] lg:rounded-[28px] lg:p-7">
          <header className="relative block min-w-0">
            {status === "success" ? <div className="flex min-w-0 items-center gap-3 pr-14"><div className="ac-success-check flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-emerald-500 text-xl font-black text-white">✓</div><h2 id="ac-lead-dialog-title" className="text-3xl font-black tracking-[-.03em]">Готово!</h2></div> : <div className="min-w-0 w-full"><div className="text-xs font-black uppercase tracking-[.16em] text-red-500">АвтоЦена</div><h2 id="ac-lead-dialog-title" className="mt-1 pr-14 text-2xl font-black tracking-[-.03em] md:text-3xl">{title}</h2><p className="mt-2 w-full max-w-none text-sm font-medium leading-6 text-[var(--ac-muted)]">{intro}</p></div>}
...
function FavoritesPinnedActions({ onLead }: { onLead: () => void }) {
  return <div data-ac-favorites-bar className="fixed inset-x-0 bottom-0 z-[9000] border-t border-white/10 bg-[var(--ac-surface)] px-3 pb-[calc(10px+env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl md:left-1/2 md:right-auto md:bottom-5 md:w-[min(720px,calc(100vw-40px))] md:-translate-x-1/2 md:rounded-[1.6rem] md:border md:p-3"><div className="mx-auto grid max-w-3xl grid-cols-2 gap-2.5"><a href="https://t.me/avtocena_bot?start=request" target="_blank" rel="noreferrer" data-offer-action="messenger" className="ac-colored-button inline-flex items-center justify-center h-14 min-w-0 rounded-[1.25rem] bg-[#00A2E8] px-3 text-[13px] font-black leading-none sm:text-sm">Чат в мессенджере</a><button type="button" onClick={onLead} className="ac-colored-button inline-flex items-center justify-center h-14 min-w-0 rounded-[1.25rem] bg-[#22B14C] px-3 text-[13px] font-black leading-none sm:text-sm">Оставить заявку</button></div></div>;
}
