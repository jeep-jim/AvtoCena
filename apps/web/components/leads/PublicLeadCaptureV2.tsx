...
function FieldLabel({ children, required = false }: { children: ReactNode; required?: boolean }) {
  return <span className="mb-1.5 block text-[11px] font-black uppercase tracking-[.13em] text-[var(--ac-muted)]">{children}{required ? <span className="ac-lead-required" aria-label="обязательное поле"> *</span> : null}</span>;
}
...
function MessengerFields({ messenger, setMessenger, contact, setContact, kind, setKind }: { messenger: MessengerKind; setMessenger: (value: MessengerKind) => void; contact: string; setContact: (value: string) => void; kind: "phone" | "username"; setKind: (value: "phone" | "username") => void }) {
  return <div className="grid gap-3">
    ...
    {kind === "phone" ? <p className="ac-messenger-callback rounded-xl px-4 py-3 text-sm font-bold leading-relaxed"><span aria-hidden="true">☝</span> Если не сможем найти вас в мессенджере по указанному номеру, перезвоним на этот номер.</p> : null}
  </div>;
}
...
function LeadDialog({ request, favorites, onClose }: { request: LeadRequest; favorites: FavoriteLeadItem[]; onClose: () => void }) {
  ...
  return createPortal(
    <div className="ac-lead-dialog-backdrop fixed inset-0 z-[20000] flex items-end justify-center bg-black/70 backdrop-blur-sm lg:items-center lg:p-6" role="dialog" aria-modal="true" aria-labelledby="ac-lead-dialog-title" onMouseDown={(event) => { if (event.target === event.currentTarget && status !== "sending") onClose(); }}>
      <div className="relative w-full lg:max-w-[760px]" onMouseDown={(event) => event.stopPropagation()}>
        <button type="button" onClick={onClose} disabled={status === "sending"} className="ac-lead-sheet-handle absolute -top-8 left-1/2 z-20 flex h-8 w-24 -translate-x-1/2 items-center justify-center lg:hidden" aria-label="Свернуть форму"><span className="block h-1.5 w-12 rounded-full bg-white/60 shadow-[0_1px_5px_rgba(0,0,0,.28)]" /></button>
        <section ref={panelRef} className="ac-lead-dialog ac-hide-scrollbar max-h-[94dvh] w-full overflow-y-auto rounded-t-[28px] bg-[var(--ac-surface)] px-4 pb-[calc(20px+env(safe-area-inset-bottom))] pt-7 text-[var(--ac-text)] lg:max-h-[90vh] lg:rounded-[28px] lg:p-7">
          <header className="relative block min-w-0">
            {status === "success" ? <div className="flex min-w-0 items-center gap-3 pr-14"><div className="ac-success-check flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-emerald-500 text-xl font-black text-white">✓</div><h2 id="ac-lead-dialog-title" className="text-3xl font-black tracking-[-.03em]">Готово!</h2></div> : <div className="min-w-0 w-full"><h2 id="ac-lead-dialog-title" className="pr-14 text-2xl font-black tracking-[-.03em] md:text-3xl">{title}</h2><p className="mt-2 w-full max-w-none text-sm font-medium leading-6 text-[var(--ac-muted)]">{intro}</p></div>}
      ...
      <style jsx global>{`
        ...
        .ac-lead-required{color:#e31b23!important;-webkit-text-fill-color:#e31b23!important}.ac-lead-error{color:#e31b23!important;-webkit-text-fill-color:#e31b23!important}.ac-success-check{color:#fff!important;-webkit-text-fill-color:#fff!important}
