  ...
  async function signIn() {
    ...
    try {
      ...
      if (!startResponse.ok || !startResult?.ok || !startResult?.telegramUrl) {
        authWindow?.close();
        const message = startResult?.error === "telegram_webhook_switch_failed"
          ? "Не удалось подготовить Telegram webhook. Попробуйте ещё раз через несколько секунд."
      ...
      for (let attempt = 0; attempt < MAX_POLLS; attempt += 1) {
        ...
        if (status?.status === "expired" || status?.status === "missing") {
          authWindow?.close();
          throw new Error("Ссылка для входа устарела. Нажмите «Войти через Telegram» ещё раз.");
        }
      }
    ...
    <div className="grid justify-items-center gap-3">
      ...
      {loading ? (
        <div className="max-w-sm text-center text-xs font-bold leading-5 text-white/50">
          В @avtocena_bot нажмите Start. После подтверждения CRM откроется здесь автоматически.
        </div>
      ) : null}
