...
const activity = [
  { text: "Запрошен расчёт Toyota Corolla из Японии", image: "/buyers/2.jpg" },
  { text: "Клиент выбрал новый Geely из Китая", image: "/buyers/5.jpg" },
  { text: "Kia Sorento переведён на этап сделки", image: "/buyers/8.jpg" },
  { text: "Автомобиль клиента прибыл во Владивосток", image: "/buyers/11.jpg" },
] as const;
...
export function DealerDemoDashboard() {
  ...
  function mainContent() {
    if (activeNav === "Заявки") {
      ...
      return <Panel title="Воронка заявок" subtitle="Перетаскивание и автоматические статусы в рабочем кабинете"><div className="grid gap-3 lg:grid-cols-4">{columns.map(([title, rows]) => <div key={title} className="rounded-2xl bg-[var(--demo-soft)] p-3"><div className="mb-3 flex items-center justify-between"><span className="text-sm font-black">{title}</span><span className="rounded-full bg-[var(--demo-avatar)] px-2 py-1 text-[10px] font-black">{rows.length}</span></div><div className="grid gap-2">{rows.map((lead) => <div key={lead.id} className="rounded-xl bg-[var(--demo-card)] p-3"><div className="text-sm font-black">{lead.client}</div><div className="mt-1 text-xs font-bold leading-5 text-[var(--demo-muted)]">{lead.car}<br />{lead.city} · {lead.time}</div></div>)}</div></div>)}</div></Panel>;
    }
    ...
    if (activeNav === "Рынки") {
      return <Panel title="Рынки и расчёт" subtitle="Единые правила расчёта и настройки компании"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{markets.map(([flag, name, cars, leads]) => <article key={name} className="rounded-2xl bg-[var(--demo-soft)] p-4"><div className="flex items-center justify-between"><span className="text-2xl">{flag}</span><span className="h-2 w-2 rounded-full bg-emerald-400" /></div><div className="mt-3 text-lg font-black">{name}</div><div className="mt-3 text-3xl font-black">{cars}</div><div className="mt-1 text-xs font-bold text-[var(--demo-muted)]">автомобилей · {leads}</div></article>)}</div></Panel>;
    }

    if (activeNav === "Компания") {
      return <Panel title="Профиль компании" subtitle="Карточка, Telegram-лента, отзывы и города работы"><div className="grid gap-4 lg:grid-cols-[1fr_.9fr]"><article className="relative rounded-2xl bg-[var(--demo-soft)] p-5"><VerifiedIcon className="absolute right-4 top-4 h-11 w-11" /><div className="flex items-center gap-4"><div className="grid h-16 w-28 place-items-center rounded-2xl bg-[#0c0e14] p-3"><img src="/brands/topavto-logo.png" alt="TopAvto" className="max-h-full max-w-full object-contain" /></div><div><div className="text-xl font-black">TopAvto</div><div className="mt-1 text-xs font-bold text-[var(--demo-muted)]">Новокузнецк · проверенный дилер</div></div></div><div className="mt-5 grid grid-cols-3 gap-2">{[["4,9", "рейтинг"], ["128", "выдач"], ["8 мин", "ответ"]].map(([value, label]) => <div key={label} className="rounded-xl bg-[var(--demo-card)] p-3 text-center"><div className="text-xl font-black">{value}</div><div className="text-[10px] font-bold text-[var(--demo-muted)]">{label}</div></div>)}</div></article><article className="rounded-2xl bg-[var(--demo-soft)] p-5"><div className="text-sm font-black">Подключённые возможности</div><div className="mt-4 grid gap-2">{["Telegram-лента", "Отзывы после сделки", "Распределение заявок", "7 рынков", "Шапка и логотип"].map((item) => <div key={item} className="flex items-center gap-2 rounded-xl bg-[var(--demo-card)] px-3 py-2 text-sm font-bold"><span className="grid h-5 w-5 place-items-center rounded-full bg-emerald-500 text-[10px] text-white">✓</span>{item}</div>)}</div></article></div></Panel>;
    }
    ...
    <div className="dealer-demo-shell mx-auto w-full max-w-[1500px] px-4 pb-14 pt-7 md:px-8 md:pb-20 md:pt-10">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm font-bold leading-6 text-[var(--demo-muted)] md:text-base">Все данные демонстрационные. Откройте каждый раздел и оцените полноценный рабочий кабинет компании.</p>
        </div>
      ...
      <section className="dealer-demo-window overflow-hidden rounded-[2rem] bg-[var(--demo-panel)]">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-white/7 px-4 py-4 md:px-6">
          <div className="flex items-center gap-3">
            <div className="grid h-12 w-24 place-items-center rounded-2xl bg-[#0c0e14] p-2"><img src="/brands/topavto-logo.png" alt="TopAvto" className="max-h-full max-w-full object-contain" /></div>
          ...
          <div className="flex items-center gap-2">
            <span className="hidden rounded-full bg-emerald-400/15 px-3 py-2 text-xs font-black text-emerald-300 sm:inline-flex">● Система работает</span>
            <img src="/avatars/manager-yellow.webp" alt="" className="h-10 w-10 rounded-full object-cover" />
          </div>
        </header>
        ...
        <div className="grid gap-4 p-4 md:p-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              {[["Новые заявки", "18", "+4 сегодня"], ["Без ответа", "3", "нужен ответ"], ["В работе", "47", "6 менеджеров"], ["В пути", "12", "2 скоро выдача"]].map(([label, value, note]) => (
                <article key={label} className="demo-metric-card rounded-[1.5rem] bg-[var(--demo-card)] p-4 md:p-5">
          ...
          <aside className="space-y-4">
            <section className="rounded-[1.6rem] bg-[var(--demo-card)] p-5">
              ...
              <div className="mt-4 grid gap-3">
                {managers.map((manager) => (
                  <button key={manager.name} type="button" onClick={() => setActiveNav("Команда")} className="flex w-full items-center justify-between rounded-2xl bg-[var(--demo-soft)] p-3 text-left">
                    <div className="flex items-center gap-3"><img src={manager.avatar} alt="" className="h-10 w-10 rounded-full object-cover" /><div><div className="text-sm font-black">{manager.name}</div><div className="text-xs font-bold text-[var(--demo-muted)]">онлайн</div></div></div><span className="text-xs font-black text-[var(--demo-muted)]">{manager.count}</span>
                  </button>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-[1.6rem] bg-[var(--demo-card)] p-5">
              <div className="flex items-center justify-between"><h2 className="text-xl font-black">Сегодня</h2><span className="text-xs font-black text-emerald-400">+12%</span></div>
              <div className="mt-4 flex h-32 items-end gap-2" aria-label="График заявок и подтверждений">
                {graph.map((bar, index) => (
                  <span key={index} className="flex flex-1 flex-col overflow-hidden rounded-t-md" style={{ height: bar.total }} title={`${bar.total} заявок, ${bar.confirmed} подтверждено`}>
                    {bar.confirmed ? <i className="block shrink-0 bg-emerald-400" style={{ height: bar.confirmed }} /> : null}
                    <i className="block min-h-0 flex-1 bg-red-500/80" />
                  </span>
                ))}
              </div>
              <div className="mt-3 flex justify-between text-[10px] font-bold text-[var(--demo-muted)]"><span>09:00</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-emerald-400" />подтверждено</span><span>Сейчас</span></div>
            </section>
          </aside>
        </div>
      </section>
      ...
      <div className={`dealer-demo-toast pointer-events-none fixed bottom-5 right-5 z-[6000] flex max-w-[360px] items-center gap-3 rounded-2xl bg-[#12151d] p-3 text-sm font-black text-white transition duration-500 ${toastVisible ? "translate-y-0 opacity-100" : "translate-y-3 opacity-0"}`}>
        <img src={activity[activityIndex].image} alt="" className="h-16 w-20 shrink-0 rounded-xl object-cover" />
        <div><div className="text-[10px] uppercase tracking-[.12em] text-emerald-300">Новый запрос</div><div className="mt-1 leading-5">{activity[activityIndex].text}</div></div>
      </div>
