...
export async function PATCH(request:Request,{params}:{params:{id:string}}){
  ...
  const stages = existing.stagesSnapshot||existing.marketStages||existing.stages||[];
  const requestedStage = body.stageIndex===""||body.stageIndex==null ? existing.stageIndex : Number(body.stageIndex);
  if(!Number.isInteger(requestedStage) || requestedStage < 0 || (stages.length && requestedStage >= stages.length)) return NextResponse.json({ok:false,error:"invalid_stage"},{status:400});
