...
export async function POST(request:Request,{params}:{params:{id:string}}) {
  ...
  const lead=(await readChunkedDataJson<any>("leads/leads.json",[])).find(row=>row.id===params.id);
  if(!lead || !canSeeLead(user,lead))return NextResponse.json({error:"lead_forbidden"},{status:403});
  try {
    const updated=await updateChunkedDataJson<any>("leads/leads.json",params.id,current=>{
