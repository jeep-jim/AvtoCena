...
export default function CrmFeedPage() {
  ...
  const leads = readDataJson<any[]>("leads/leads.json", []).map((lead) => ({
    id: lead.id,
  ...
  const clients = readDataJson<any[]>("clients/clients.json", []).map((client) => ({
    id: client.id,
  ...
  return (
    <CrmShell title="Лента" subtitle="Общая лента действий и личная лента менеджера.">
