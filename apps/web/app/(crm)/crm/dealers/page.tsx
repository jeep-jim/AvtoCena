...
export default async function CrmDealersPage() {
  ...
  return (
    <CrmShell
      ...
      title="Дилеры"
      subtitle="Компании, которые работают в платформе, их города, рынки и заявки на подключение."
    >
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {dealers.map((dealer) => (
          <article key={dealer.id} className="glass rounded-[1.6rem] p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-xs font-black uppercase tracking-[.12em] text-red-400">{dealer.pilot ? "Пилот" : "Дилер"}</div>
                <h2 className="mt-2 text-2xl font-black">{dealer.name}</h2>
            ...
            <div className="mt-5 rounded-2xl bg-white/[.045] p-4">
              <div className="text-xs font-bold text-white/38">Рынки</div>
              <div className="mt-2 text-sm font-black leading-6 text-white/80">{Array.isArray(dealer.markets) ? dealer.markets.join(" · ") : "—"}</div>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2 text-sm font-bold">
              ...
              <div className="rounded-2xl bg-white/[.045] p-3"><div className="text-xs text-white/38">Активные заявки</div><div className="mt-1 text-2xl font-black">{dealer.activeLeads || 0}</div></div>
            </div>
          </article>
        ))}
      </section>
      ...
      <section className="glass mt-6 rounded-[1.8rem] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-2xl font-black">Заявки на подключение</h2>
            <p className="mt-1 text-sm font-bold text-white/48">Новые компании с публичного лендинга для дилеров.</p>
          </div>
          <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-black text-white/60">{sortedApplications.length}</span>
        </div>
        ...
        <div className="mt-5 grid gap-3">
          {sortedApplications.length ? sortedApplications.map((item) => (
            <article key={item.id} className="grid gap-3 rounded-2xl bg-white/[.045] p-4 md:grid-cols-[1.1fr_.8fr_.8fr_auto] md:items-center">
              <div>
                <div className="font-black text-white">{item.companyName}</div>
                <div className="mt-1 text-sm font-bold text-white/45">{item.contactName} · {item.city}</div>
              </div>
              <div className="text-sm font-bold text-white/65">
                <div>{item.phone}</div>
                <div className="mt-1 text-white/40">{item.telegram || "Telegram не указан"}</div>
              </div>
              <div className="text-sm font-bold text-white/55">
                <div>{item.teamSize ? `${item.teamSize} сотрудников` : "Размер команды не указан"}</div>
                <div className="mt-1 line-clamp-2">{item.markets || "Рынки не указаны"}</div>
              </div>
