  ...
  return <CrmShell activeHref="/crm/clients" title="Клиенты" subtitle="Клиентская база, контакты, документы и история обращений.">
    <div className="crm-clients-layout">
      <section className="min-w-0" aria-label="Клиентская база">
        ...
        <div className="crm-client-results"><span>Найдено: {clients.length}</span>{(q || managerId) && <Link href="/crm/clients" className="underline">Сбросить поиск</Link>}<div id="crm-client-view-controls"/></div>
        <ClientView>
      ...
      <aside className="crm-client-create" aria-label="Добавить клиента">{hasCrmPermission(user,"editClients")?<ClientCreatePanel />:<p className="text-sm text-[var(--ac-muted)]">Доступ к клиентской базе: просмотр.</p>}</aside>
    </div>
  </CrmShell>;
}
