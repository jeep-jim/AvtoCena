import { photoProxyEligible, validPhotoSignature } from "@/lib/catalog/photo-proxy-policy";
...
export async function GET(request: Request, {params: routeParams}: {params: Promise<{signature: string}>}) {
  ...
  try {
    const image = await cache.read(url, async () => {
      ...
      if (!response.ok || !/^image\/(jpeg|png|webp)(;|$)/i.test(response.headers.get("content-type") || "") || Number(response.headers.get("content-length")) > 8 * 1024 * 1024) { await response.body?.cancel(); throw new Error("photo_invalid"); }
