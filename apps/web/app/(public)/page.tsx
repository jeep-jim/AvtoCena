  ...
  return (
    <section className="mt-4 grid gap-5 rounded-[1.6rem] border border-white/10 bg-white/[0.04] p-5 md:grid-cols-[minmax(0,1fr)_290px] md:items-center md:p-6">
      <div>
        ...
        <p className="mt-3 max-w-4xl text-sm font-medium leading-7 text-white/58 md:text-[15px]">
          Сервис помогает быстро понять, какой автомобиль можно привезти из Японии, Китая, Кореи, ОАЭ или Европы. Вы задаёте базовые параметры, а система показывает реальные варианты для следующего шага с менеджером TopAvto.
        </p>
      </div>
      ...
      <button type="button" onClick={() => onOpen(offer)} className="block w-full text-left">
        <div className="relative h-56 overflow-hidden bg-white/[0.04]">
          ...
          <div className="absolute bottom-3 left-3 right-3">
            ...
            <div className="mt-2 flex gap-2 text-[11px] font-black text-white/85">
              <span className="rounded-full bg-black/40 px-3 py-1 backdrop-blur">{offer.year}</span>
              <span className="rounded-full bg-black/40 px-3 py-1 backdrop-blur">{offer.mileage}</span>
            </div>
          </div>
        </div>
        <div className="p-4">
          <div className="flex items-end justify-between gap-3">
            <div>
              <div className="text-[10px] font-black uppercase tracking-[0.16em] text-white/36">Ориентир</div>
              <div className="mt-1 text-2xl font-black tracking-[-0.04em]">{money(offer.price)} ₽</div>
            </div>
            <span className="rounded-xl bg-white/[0.07] px-3 py-2 text-xs font-black">Подробнее</span>
          </div>
          <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-white/58">
            <span className="rounded-full bg-white/[0.05] px-3 py-1.5">{offer.engine}</span>
            <span className="rounded-full bg-white/[0.05] px-3 py-1.5">{offer.power}</span>
          </div>
        </div>
      </button>
...
function CostLines({ offer }: { offer: HomeOffer }) {
  ...
  return (
    <div className="grid gap-2.5 text-sm font-bold text-white/68">
      {lines.map(([label, amount]) => (
        <div key={label} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-baseline gap-2">
          <span className="text-red-300/65">·</span>
          <span className="flex min-w-0 items-baseline gap-2"><span className="shrink-0">{label}</span><span className="mb-1 min-w-5 flex-1 border-b border-dotted border-white/10" /></span>
          <span className="min-w-[105px] text-right font-black text-white">{money(amount)} ₽</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="relative z-10 max-h-[100dvh] w-full overflow-y-auto bg-[#0b0d14] md:max-h-[calc(100dvh-40px)] md:max-w-5xl md:rounded-[2rem]">
        <div className="relative h-[330px] overflow-hidden sm:h-[430px]">
          ...
          <div className="absolute left-4 top-4 flex flex-wrap gap-2 sm:left-6 sm:top-6">
            <span className="rounded-full bg-red-500 px-3 py-1 text-xs font-black">{offer.countryLabel}</span>
            <span className="rounded-full bg-black/40 px-3 py-1 text-xs font-black backdrop-blur">{offer.bodyLabel}</span>
            <span className="rounded-full bg-black/40 px-3 py-1 text-xs font-black backdrop-blur">{offer.year}</span>
          </div>
          ...
          {images.length > 1 ? (
            <>
              <button type="button" onClick={previous} className="absolute left-3 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/50 text-2xl backdrop-blur">‹</button>
              <button type="button" onClick={next} className="absolute right-3 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/50 text-2xl backdrop-blur">›</button>
            </>
          ) : null}
          <div className="absolute bottom-5 left-4 right-4 sm:bottom-7 sm:left-7 sm:right-7">
            <h2 className="max-w-3xl text-[30px] font-black leading-[.98] tracking-[-0.045em] sm:text-[46px]">{offer.title}</h2>
            <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-white/68"><span>{offer.engine}</span><span>·</span><span>{offer.power}</span></div>
          </div>
        </div>

        {images.length > 1 ? (
          <div className="ac-hide-scrollbar flex gap-2 overflow-x-auto px-4 pt-3 sm:px-6">
            {images.map((image, index) => <button key={`${image}-${index}`} type="button" onClick={() => setPhotoIndex(index)} className={`h-16 w-24 shrink-0 overflow-hidden rounded-xl border ${index === photoIndex ? "border-red-400" : "border-white/10 opacity-65"}`}><img src={image} alt="" className="h-full w-full object-cover" /></button>)}
          </div>
        ) : null}

        <div className="grid gap-6 p-5 sm:p-7 md:grid-cols-[minmax(0,1fr)_310px]">
          <div>
            <p className="text-sm font-medium leading-7 text-white/58">Ориентировочная структура цены. Финальная стоимость зависит от курса, состояния автомобиля, даты покупки, логистики и города доставки.</p>
            <div className="mt-5"><CostLines offer={offer} /></div>
          </div>
          <div className="md:border-l md:border-white/8 md:pl-6">
            <div className="text-xs font-black uppercase tracking-[0.18em] text-white/38">Ориентир цены</div>
            <div className="mt-2 text-4xl font-black tracking-[-0.05em]">{money(offer.price)} ₽</div>
            <div className="mt-1 text-sm font-black text-emerald-300">по текущему варианту</div>
            <button type="button" onClick={() => onRequest(offer)} className="avto-button mt-6 h-14 w-full rounded-2xl font-black">Получить предложение</button>
            <Link href={`/cars/offer/${offer.id}`} className="mt-3 block rounded-2xl bg-white/[0.06] px-5 py-4 text-center font-black">Открыть карточку</Link>
          </div>
        </div>
      </div>
    </div>,
...
export default function HomePage() {
      ...
      .then((data) => {
        ...
        setOffers(items.slice(0, 12).map((raw: any) => {
          ...
          return {
            ...
            price: o.totalRub || 0,
            country: o.market,
            countryLabel: o.marketLabel,
            body: o.bodyType || "",
            bodyLabel: o.bodyLabel,
            mileageKm: o.mileageKm,
            mileage: o.mileageKm ? `${money(o.mileageKm)} км` : "пробег уточняется",
            engine: o.engineCc ? `${o.engineCc} см³` : `${o.fuelLabel}`,
  ...
  function buildResultsUrl(extra?: Partial<HomeOffer>) {
    ...
    if (finalYear === "older") params.set("yearTo", "2017");
    else if (finalYear) params.set("yearFrom", String(finalYear));
  ...
  return (
    <main className="min-h-screen bg-[#07080d] text-white">
      ...
      <div className="mx-auto w-full max-w-[1500px] px-4 pb-16 md:px-8">
        <section className="grid items-start gap-7 py-7 lg:grid-cols-[minmax(0,1fr)_420px] lg:gap-10 lg:py-12">
          <div>
            ...
            <div className="mt-7 hidden grid-cols-1 gap-4 lg:grid">
              {benefits.map((item) => <div key={item.title} className="flex items-center gap-4"><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-red-400/25 bg-red-500/10 text-xl text-red-400">{item.icon}</div><div><div className="font-black">{item.title}</div><div className="mt-1 text-sm text-white/45">{item.text}</div></div></div>)}
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 border-y border-white/8 py-5 lg:hidden">
          {benefits.map((item) => <div key={item.title} className="text-center"><div className="mx-auto flex h-11 w-11 items-center justify-center rounded-xl bg-red-500/12 text-xl text-red-400">{item.icon}</div><div className="mt-2 text-sm font-black">{item.title}</div></div>)}
        </div>
        ...
        <section className="mt-6 border-t border-red-500/45 pt-6" data-demo-enabled={allowDemo ? "true" : "false"}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="text-xs font-black uppercase tracking-[0.18em] text-red-300">Свежие предложения</div>
            ...
            <div className="grid gap-3 sm:grid-cols-[180px_220px_auto]">
              <SelectBox value={catalogMarket} onChange={setCatalogMarket} ariaLabel="Фильтр по стране">{marketOptions.map((item) => <option key={item.value || "any"} value={item.value}>{item.label}</option>)}</SelectBox>
              <input value={catalogMake} onChange={(event) => setCatalogMake(event.target.value)} placeholder="Марка или модель" className="soft-input h-14 rounded-2xl bg-[#181a21] px-4 text-sm font-bold outline-none" />
              <Link href="/cars" className="avto-button flex h-14 items-center justify-center rounded-2xl px-5 font-black">Смотреть все</Link>
            </div>
          </div>

          {catalogOffers.length ? (
            <div className="ac-hide-scrollbar mt-6 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 md:grid md:grid-cols-2 md:overflow-visible lg:grid-cols-3">
              {catalogOffers.slice(0, 6).map((offer) => <OfferCard key={offer.id} offer={offer} onOpen={setSelectedOffer} />)}
            </div>
          ) : (
            <div className="mt-6 rounded-[1.6rem] bg-white/[0.045] p-6 text-sm font-bold text-white/58">Каталог обновляется. Оставьте заявку — менеджер подберёт автомобиль.</div>
          )}
        </section>
      </div>
