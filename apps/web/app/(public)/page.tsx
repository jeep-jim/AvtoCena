      ...
      <div className="buyer-gallery-mask mt-5 overflow-hidden">
        <div className="buyer-gallery-track flex w-max gap-3 md:gap-4">
          {photos.map((photo, index) => (
            <button key={`${photo.src}-${index}`} type="button" onClick={() => setSelected(index % buyerPhotos.length)} className="h-[150px] w-[220px] shrink-0 overflow-hidden rounded-[1.35rem] bg-white/[0.04] sm:h-[180px] sm:w-[270px] lg:h-[205px] lg:w-[315px]">
...
function ExecutorBlock() {
  return (
    <section className="ac-executor-block mt-4 grid gap-5 rounded-[1.6rem] p-5 md:grid-cols-[minmax(0,1fr)_290px] md:items-center md:p-6">
      <div>
        <h3 className="text-xl font-black">АвтоЦена — подбор автомобиля под ваш бюджет</h3>
        <p className="mt-3 max-w-4xl text-sm font-medium leading-7 text-white/58 md:text-[15px]">Сервис помогает быстро понять, какой автомобиль можно привезти из Японии, Китая, Кореи, ОАЭ или Европы. Вы задаёте параметры, а система показывает реальные варианты и актуальный расчёт.</p>
      </div>
      <div className="ac-executor-logo flex min-h-32 items-center justify-center rounded-[1.3rem] p-5">
        <img src="/brands/topavto-logo.png" alt="TopAvto" className="ac-topavto-logo max-h-24 w-full object-contain" />
      </div>
    </section>
  );
}
...
function MobileExchangeRates({ rates }: { rates: PublicRate[] }) {
  ...
  const format = (currency: string, amount: number) => {
    const effectiveRate = Number(rateMap.get(currency) || 0);
    if (!effectiveRate) return "—";
    return new Intl.NumberFormat("ru-RU", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(effectiveRate * amount);
  };

  return (
    <section className="ac-mobile-rates grid grid-cols-5 gap-1 rounded-[1.4rem] px-2 py-4 lg:hidden" aria-label="Текущие курсы валют рынков">
      {mobileRateDefinitions.map((item) => (
        <div key={item.currency} className="flex min-w-0 flex-col items-center text-center" title={`${item.label}: ${item.currency}`}>
          <span className="text-[24px] leading-none" aria-hidden="true">{item.flag}</span>
          <span className="mt-1 text-[9px] font-black uppercase tracking-[0.08em] text-white/48">{item.currency}</span>
          <span className="mt-1 text-[10px] font-black leading-[1.15] sm:text-[11px]">{item.unit}</span>
          <span className="mt-0.5 text-[10px] font-bold leading-[1.15] text-white/58 sm:text-[11px]">{format(item.currency, item.amount)} ₽</span>
        </div>
      ))}
    </section>
  );
...
function mapItem(raw: any): CatalogItem {
  ...
  return {
    ...
    market: o.market,
    year: o.year,
    body: String(raw.bodyType || ""),
    totalRub: Number(o.totalRub || 0),
  };
}
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const requests = [
      fetch("/api/catalog/search?pageSize=12&sort=updatedAt&includeRates=1", { cache: "no-store" }).then((response) => response.ok ? response.json() : null),
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      setFoundCount(null);
      fetch(`/api/catalog/search?${params}`, { cache: "no-store" })
        .then((response) => response.ok ? response.json() : null)
        .then((data) => setFoundCount(Number(data?.total || 0)))
    ...
    <main className="ac-page-copy min-h-screen overflow-x-hidden bg-[#07080d] text-white">
      ...
      <div className="mx-auto w-full max-w-[1500px] px-4 pb-16 md:px-8">
        <section className="grid items-start gap-7 py-7 lg:grid-cols-[minmax(0,1fr)_420px] lg:gap-10 lg:py-12">
            ...
            <div className="mt-7 hidden grid-cols-1 gap-4 lg:grid">
              {benefits.map((item) => (
                <div key={item.title} className="flex items-center gap-4">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-red-500/10 text-red-400"><BenefitIcon type={item.icon} /></div>
          ...
          <div id="form" className="ac-filter-panel rounded-[1.8rem] bg-white/[0.075] p-4 backdrop-blur-xl md:p-5">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="text-xs font-black uppercase tracking-[0.16em] text-red-200">Бюджет</div>
              <div className="flex items-center gap-2 rounded-full bg-black/18 px-3 py-1.5 text-[11px] font-black text-white/72">
        ...
        <section className="mt-6 pt-6" data-demo-enabled={allowDemo ? "true" : "false"}>
          ...
          {catalogItems.length ? (
            <div className="ac-hide-scrollbar mt-6 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 md:grid md:grid-cols-2 md:overflow-visible lg:grid-cols-3">
              {catalogItems.map((item) => <div key={item.id} className="w-[82vw] max-w-[360px] shrink-0 snap-start md:w-auto md:max-w-none"><CatalogCard offer={item.raw} compact /></div>)}
            </div>
          ) : (
            <div className="mt-6 rounded-[1.6rem] bg-white/[0.045] p-6 text-sm font-bold text-white/58">Каталог обновляется. Свежие варианты появятся после импорта рынка.</div>
          )}
        </section>
      </div>
    </main>
