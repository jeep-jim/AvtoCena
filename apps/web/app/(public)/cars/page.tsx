...
export default async function CarsPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const green = overviewEligible ? await readGreenCorner().catch(()=>null) : null;
  ...
  return <main className="ac-catalog-page ac-page-copy min-h-screen bg-[#0f172a] text-white">
    ...
    <section className="mx-auto w-full max-w-[1500px] px-4 py-6 md:px-8 md:py-10">
      ...
      <p className="ac-catalog-result-count mt-3 flex min-w-0 items-center gap-2 text-sm font-bold leading-6 text-[var(--ac-text)] md:text-base" role="status" aria-live="polite" aria-atomic="true" data-catalog-result-count={total}>
