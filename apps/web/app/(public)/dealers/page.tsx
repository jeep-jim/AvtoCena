...
export default async function DealersLandingPage({
  searchParams,
}: {
    ...
    <main className="ac-page-copy min-h-screen bg-[#07080d] text-white">
      ...
      <section className="relative overflow-hidden border-b border-white/7">
        ...
        <div className="relative mx-auto grid w-full max-w-[1500px] gap-10 px-4 py-14 md:px-8 md:py-20 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-4 max-w-5xl text-[43px] font-black leading-[.96] tracking-[-.055em] sm:text-6xl lg:text-[78px]">
              Первая CRM для компаний по привозу автомобилей
            </h1>
            <p className="mt-6 max-w-3xl text-base font-bold leading-7 text-white/62 md:text-xl md:leading-8">
              Ведите клиентов, управляйте менеджерами, рассчитывайте автомобили и получайте новые заявки в одной системе. АвтоЦена объединяет покупателей и проверенных дилеров по всей России.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#connect" className="rounded-2xl bg-[#ff353d] px-6 py-4 text-base font-black text-white transition hover:brightness-110">Подключиться бесплатно</a>
            ...
            <div className="mt-8 grid max-w-3xl grid-cols-2 gap-3 sm:grid-cols-4">
              {[['0 ₽', 'подключение CRM'], ['7', 'рынков в расчёте'], ['1 500 ₽', 'проверенный профиль'], ['Только факт', 'комиссия с нашей сделки']].map(([value, label]) => (
                <div key={label} className="rounded-2xl bg-white/[.055] p-4">
                  ...
                  <div className="mt-1 text-xs font-bold leading-5 text-white/44">{label}</div>
                </div>
              ))}
            </div>
          </div>

          <div className="rounded-[2rem] border border-white/8 bg-[#12151d] p-5 md:p-7">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {[
                ...
                ["Без ответа", "3"],
                ["Клиенты в работе", "47"],
                ["Автомобили в пути", "12"],
              ].map(([label, value]) => <div key={label} className="rounded-2xl bg-white/[.055] p-4"><div className="text-xs font-bold text-white/42">{label}</div><div className="mt-2 text-4xl font-black">{value}</div></div>)}
            </div>
            <div className="mt-3 rounded-2xl bg-white/[.055] p-4">
              <div className="flex items-center justify-between text-sm font-black"><span>Заявка из Новокузнецка</span><span className="text-emerald-400">новая</span></div>
              <div className="mt-2 text-sm font-bold leading-6 text-white/52">Кроссовер до 2 млн ₽, левый руль, не старше 2021 года. Запрос принят Telegram-ботом и рассчитан автоматически.</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="how" className="border-y border-white/7 bg-[#0c0e14]">
        <div className="mx-auto w-full max-w-[1500px] px-4 py-14 md:px-8 md:py-20">
          <div className="max-w-4xl">
            <div className="text-sm font-black uppercase tracking-[.18em] text-red-400">Как Яндекс Еда, только в автотематике</div>
            <h2 className="mt-3 text-4xl font-black tracking-[-.05em] md:text-6xl">АвтоЦена приводит клиента и управляет всей сделкой</h2>
          </div>
          <div className="mt-9 grid gap-3 lg:grid-cols-5">
            {workflow.map(([title, text], index) => <article key={title} className="rounded-[1.6rem] bg-white/[.055] p-5"><div className="grid h-10 w-10 place-items-center rounded-xl bg-red-500 text-sm font-black">{index + 1}</div><h3 className="mt-4 text-lg font-black">{title}</h3><p className="mt-3 text-sm font-bold leading-6 text-white/48">{text}</p></article>)}
          </div>
        </div>
      </section>

      <section className="mx-auto grid w-full max-w-[1500px] gap-5 px-4 py-14 md:px-8 md:py-20 lg:grid-cols-2">
        <article className="rounded-[2rem] bg-[#12151d] p-6 md:p-8">
          <div className="text-sm font-black uppercase tracking-[.16em] text-white/42">Бесплатно</div>
          <h2 className="mt-3 text-4xl font-black tracking-[-.045em]">CRM для ежедневной работы</h2>
          <p className="mt-4 text-sm font-bold leading-6 text-white/52">Дилер может вести собственную клиентскую базу и сотрудников без комиссии АвтоЦена.</p>
          <ul className="mt-6 grid gap-3">{freeFeatures.map((item) => <Check key={item}>{item}</Check>)}</ul>
          <a href="#connect" className="mt-7 inline-flex rounded-2xl bg-white px-5 py-3.5 text-sm font-black text-black">Начать бесплатно</a>
        </article>

        <article className="rounded-[2rem] border border-red-500/35 bg-[linear-gradient(145deg,rgba(255,53,61,.18),rgba(18,21,29,1)_48%)] p-6 md:p-8">
          <div className="text-sm font-black uppercase tracking-[.16em] text-red-300">Проверенный дилер</div>
          <div className="mt-3 flex items-end gap-2"><div className="text-5xl font-black tracking-[-.05em]">1 500 ₽</div><div className="pb-1 text-sm font-bold text-white/44">в месяц</div></div>
          <p className="mt-4 text-sm font-bold leading-6 text-white/58">Подтверждённый профиль получает доверие клиентов и участвует в распределении заявок по своему городу.</p>
          <ul className="mt-6 grid gap-3">{verifiedFeatures.map((item) => <Check key={item}>{item}</Check>)}</ul>
          <div className="mt-7 rounded-2xl bg-black/25 px-4 py-3 text-sm font-black text-white/78">Комиссия — только с клиента, которого привела АвтоЦена, и только после результата.</div>
        </article>
      </section>
      ...
      <section className="border-y border-white/7 bg-[#0c0e14]">
        <div className="mx-auto grid w-full max-w-[1500px] gap-8 px-4 py-14 md:px-8 md:py-20 lg:grid-cols-2 lg:items-center">
          <div>
            <div className="text-sm font-black uppercase tracking-[.18em] text-red-400">Профиль компании</div>
            <h2 className="mt-3 text-4xl font-black tracking-[-.05em] md:text-6xl">Мини-сайт дилера внутри АвтоЦена</h2>
            <p className="mt-5 text-base font-bold leading-7 text-white/56">Логотип, города, рынки, фотографии офиса и выдач, отзывы подтверждённых клиентов, рейтинг и новости из Telegram. На главной пользователь увидит проверенных дилеров именно в своём городе.</p>
          </div>
          <div className="rounded-[2rem] bg-[#151821] p-5 md:p-7">
            <div className="flex items-center gap-4"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-white text-lg font-black text-black">TA</div><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-2xl font-black">TopAvto</h3><span className="rounded-full bg-emerald-400/15 px-3 py-1 text-xs font-black text-emerald-300">Проверен АвтоЦена</span></div><div className="mt-1 text-sm font-bold text-white/44">Новокузнецк · 7 рынков</div></div></div>
            <div className="mt-5 grid grid-cols-3 gap-2">{[['4,9', 'рейтинг'], ['128', 'выдач'], ['8 мин', 'ответ']].map(([value, label]) => <div key={label} className="rounded-2xl bg-white/[.055] p-3 text-center"><div className="text-xl font-black">{value}</div><div className="mt-1 text-[11px] font-bold text-white/38">{label}</div></div>)}</div>
            <div className="mt-3 rounded-2xl bg-white/[.055] p-4 text-sm font-bold leading-6 text-white/56">Новая выдача: автомобиль доставлен клиенту из Японии. Фото и публикация автоматически добавлены из Telegram-канала компании.</div>
          </div>
        </div>
      </section>
      ...
      <section id="connect" className="mx-auto grid w-full max-w-[1500px] gap-8 px-4 py-14 md:px-8 md:py-20 lg:grid-cols-[.8fr_1.2fr]">
        <div>
          <div className="text-sm font-black uppercase tracking-[.18em] text-red-400">Пилотный запуск</div>
          <h2 className="mt-3 text-4xl font-black tracking-[-.05em] md:text-6xl">Подключите свою компанию</h2>
          <p className="mt-5 text-base font-bold leading-7 text-white/56">Первый пилот проходит на TopAvto. Следующие дилеры получат CRM, профиль компании и возможность первыми принимать заявки АвтоЦена в своём городе.</p>
          <div className="mt-6 rounded-2xl bg-white/[.055] p-4 text-sm font-bold leading-6 text-white/56">После заявки мы свяжемся, подтвердим город и рынки, создадим рабочее пространство компании и поможем добавить менеджеров.</div>
        </div>
        ...
        <form action="/api/dealers/apply" method="post" className="rounded-[2rem] bg-[#12151d] p-5 md:p-7">
          ...
          <button className="mt-5 w-full rounded-2xl bg-[#ff353d] px-5 py-4 text-base font-black text-white transition hover:brightness-110">Отправить заявку</button>
        </form>
      </section>

      <footer className="border-t border-white/7 px-4 py-8 text-center text-sm font-bold text-white/36">
        <Link href="/" className="hover:text-white">АвтоЦена</Link> · CRM, расчёты и новые клиенты для автомобильных дилеров
      </footer>
    </main>
  );
}
