  ...
  return (
    <main className="ac-dealers-page ac-page-copy min-h-screen bg-[#07080d] text-white">
          ...
          <div>
            ...
            <div className="mt-8 grid max-w-3xl grid-cols-2 gap-3 sm:grid-cols-4">
              {[["0 ₽", "подключение CRM"], ["7", "рынков в расчёте"], ["1 500 ₽", "проверенный профиль"], ["Только факт", "комиссия с нашей сделки"]].map(([value, label]) => (
                <div key={label} className="rounded-2xl bg-white/[.055] p-4"><div className="text-xl font-black text-white">{value}</div><div className="mt-1 text-xs font-bold leading-5 text-white/44">{label}</div></div>
              ))}
            </div>
          </div>
          <DealerHeroPreview />
      ...
      <section className="mx-auto grid w-full max-w-[1500px] gap-5 px-4 py-14 md:px-8 md:py-20 lg:grid-cols-2">
        ...
        <article className="rounded-[2rem] border border-red-500/35 bg-[linear-gradient(145deg,rgba(255,53,61,.18),rgba(18,21,29,1)_48%)] p-6 md:p-8"><div className="text-sm font-black uppercase tracking-[.16em] text-red-300">Проверенный дилер</div><div className="mt-3 flex items-end gap-2"><div className="text-5xl font-black tracking-[-.05em]">1 500 ₽</div><div className="pb-1 text-sm font-bold text-white/44">в месяц</div></div><p className="mt-4 text-sm font-bold leading-6 text-white/58">Подтверждённый профиль получает доверие клиентов и участвует в распределении заявок по своему городу.</p><ul className="mt-6 grid gap-3">{verifiedFeatures.map((item) => <Check key={item}>{item}</Check>)}</ul><div className="mt-7 rounded-2xl bg-black/25 px-4 py-3 text-sm font-black text-white/78">Комиссия — только с клиента, которого привела АвтоЦена, и только после результата.</div></article>
      </section>
      ...
      <section className="border-y border-white/7 bg-[#0c0e14]">
        <div className="mx-auto grid w-full max-w-[1500px] gap-8 px-4 py-14 md:px-8 md:py-20 lg:grid-cols-2 lg:items-center">
          ...
          <div className="rounded-[2rem] bg-[#151821] p-5 md:p-7"><div className="flex items-center gap-4"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-white text-lg font-black text-black">TA</div><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-2xl font-black">TopAvto</h3><span className="rounded-full bg-emerald-400/15 px-3 py-1 text-xs font-black text-emerald-300">Проверен АвтоЦена</span></div><div className="mt-1 text-sm font-bold text-white/44">Новокузнецк · 7 рынков</div></div></div><div className="mt-5 grid grid-cols-3 gap-2">{[["4,9", "рейтинг"], ["128", "выдач"], ["8 мин", "ответ"]].map(([value, label]) => <div key={label} className="rounded-2xl bg-white/[.055] p-3 text-center"><div className="text-xl font-black">{value}</div><div className="mt-1 text-[11px] font-bold text-white/38">{label}</div></div>)}</div><div className="mt-3 rounded-2xl bg-white/[.055] p-4 text-sm font-bold leading-6 text-white/56">Новая выдача: автомобиль доставлен клиенту из Японии. Фото и публикация автоматически добавлены из Telegram-канала компании.</div></div>
        </div>
      </section>
