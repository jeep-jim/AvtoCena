...
test("global cleanup is a manual recovery tool and audits all six markets", () => {
...
test("Japan strict writer is dispatch-only and proves all-seven safety after every write", () => {
  ...
  assert.match(japanStrictWorkflow, /CATALOG_AUDIT_ASSERT_MARKETS: korea,china,japan,uae,europe,georgia/);
...
test("K Car repair post-write audit uses the canonical model-year cap and no stale market floors", () => {
  assert.match(kcarRepairWorkflow, /CATALOG_AUDIT_ASSERT_MARKETS: korea,china,japan,uae,europe,georgia/);
