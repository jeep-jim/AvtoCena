...
test("recovery preservation gates keep untouched markets byte-stable and canonicalize only mutable rows", () => {
  ...
  assert.match(storage, /canonicalizePublicCatalogOffers\(publicOffers, exactPreserveMarkets, protectedPublicIds, options.retainedPowerMixIds, options.retainedPowerMixMinimumByMarket\)/);
...
test("standard one-market publisher expires stale target rows, reapplies quality policy, and preserves untouched markets during canonicalization", () => {
  ...
  assert.match(storage, /canonicalizePublicCatalogOffers\(publicOffers, exactPreserveMarkets, protectedPublicIds, options.retainedPowerMixIds, options.retainedPowerMixMinimumByMarket\)[\s\S]*beforePublishValidate\(publishedOffers\)[\s\S]*const generationId/);
