...
import { offerPdfData, offerPdfNotes, renderOfferPdf } from "../apps/web/lib/catalog/offer-pdf";
...
test("PDF uses current draft, engine totals and a separate deposit, not the template amounts",()=>{
 ...
 assert.equal(data.sections[1].rows.at(-1)?.value,"820 000 ₽");
});
...
test("zero commission is a valid completed calculation",()=>{
 ...
 assert.equal(data.sections[1].rows.find(r=>r.label.includes("Комиссия компании"))?.value,"0 ₽");
});
...
test("dealer layout relocates the advance and commission without charging the advance twice",()=>{
 const data=offerPdfData(offer,draft,calculation);
