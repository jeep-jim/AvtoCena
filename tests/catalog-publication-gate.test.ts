...
test("Catalog V2 publishes each market independently and preserves completed markets", () => {
  ...
  assert.match(marketPublisher, /hasAllowedCatalogSourceProvenance, isCrediblePublicOffer, isCatalogYearAllowed/);
  assert.match(marketPublisher, /purgedForbiddenPublicByMarket/);
  assert.match(marketPublisher, /purgedForbiddenInternalByMarket/);
  assert.match(marketPublisher, /const replaceInternalSourceIds = new Set/);
  assert.match(marketPublisher, /replaceInternalSourceIds,/);
  assert.doesNotMatch(marketPublisher, /iterateOffersForMaintenance/);
  assert.doesNotMatch(marketPublisher, /preservedInternalOffers:/);
