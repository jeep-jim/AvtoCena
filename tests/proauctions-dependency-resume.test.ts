...
for(const dependency of ['photo','witness','legacy-photo'])test(`${dependency} refusal preserves the lot across a real collector restart`,async()=>{
 ...
 const html=['ProAuctions','Статистика','Mazda','CX-5'].map(value=>`<span itemprop="name">${value}</span>`).join('')+await fs.readFile(path.join(repo,'tests/fixtures/proauctions/30162134.html'),'utf8');
...
globalThis.fetch=async(input)=>{
 ...
 if(/^https:\\/\\/jp\\d+\\.pa-server\\.ru\\//.test(url))return process.env.FAIL_DEPENDENCY==='photo'?new Response('',{status:403}):new Response(await fs.readFile(String(image++%3)+'.png'));
