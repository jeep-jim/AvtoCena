...
test("заблокированный менеджер не входит в CRM", () => {
  assert.equal(isBlockedManager({ status: "blocked" }), true);
});

test("одноразовое приглашение нельзя использовать дважды: состояние usedAt блокирует повтор", () => {
  const invite = { hash: "secret-hash", expiresAt: "2099-01-01", usedAt: "2026-07-13T10:00:00Z" };
...
test("формирование договора подставляет данные клиента из mapping модели", () => {
  const client = { fio: "Степан Иванов", passport: "1234 567890", phone: "+7999" };
  const result = `Клиент: ${client.fio}; Паспорт: ${client.passport}; Телефон: ${client.phone}`;
  assert.match(result, /Степан Иванов/);
});
