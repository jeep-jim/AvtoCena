...
test("10k rebuild requires a buyer-usable multi-photo gallery and caps galleries at 30", () => {
  assert.doesNotMatch(importer, /CATALOG_REBUILD_PREFERRED_IMAGES_PER_OFFER\s*=\s*["']30["']/);
  assert.match(workflow, /CATALOG_REBUILD_MIN_IMAGES_PER_OFFER: "2"/);
  assert.match(workflow, /CATALOG_REBUILD_PREFERRED_IMAGES_PER_OFFER: "30"/);
  assert.doesNotMatch(workflow, /CATALOG_COLLECTION_IMAGE_LIMIT: "5"/);
