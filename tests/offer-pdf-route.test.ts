...
test("PDF endpoint protects staff access and exports both incomplete and unsaved drafts without writing",async()=>{
 ...
 const sources:Record<string,string>={
  ...
  "@/lib/catalog/offer-pdf":`export const offerPdfData=(o,d,c,w)=>({draft:d,calculation:c,warning:w,url:"https://avtocena.com/cars/offer/lot"});export const renderOfferPdf=async d=>{globalThis.__pdfRoute.renders++;globalThis.__pdfRoute.data=d;return Buffer.from('%PDF-test');};`
 };
 ...
 try{
  ...
  state.saved={version:"saved-v1",draft:{year:"2021",fuel:"petrol",engineCc:"1998",powerHp:"150"},calculation:{totalRub:2600000}};
  assert.equal((await post(request())).status,200);assert.equal(state.data.calculation.totalRub,2600000);assert.match(state.data.url,/calculation=saved-v1/);
