...
test('activity visibility follows current assignment, management scope and identity without leaking hidden events',async()=>{
 ...
 const r=await build({entryPoints:['apps/web/lib/crm-activity.ts'],bundle:true,platform:'node',format:'cjs',write:false,packages:'external',plugins:[{name:'activity-storage',setup(b){b.onResolve({filter:/^\.\/(data|crm-users|crm-notification-store)$/},a=>({path:a.path,namespace:'mock'}));b.onLoad({filter:/.*/,namespace:'mock'},a=>({contents:a.path==='./crm-notification-store'?`export const notifyTeam=async()=>{};`:a.path==='./data'?`const s=globalThis.__activity;export const appendChunkedDataJson=async(p,e)=>{s.events.unshift(e);return e};export const generateId=()=>"id";export const readChunkedDataJson=async p=>p.startsWith('leads')?s.leads:s.clients;export const readRecentChunkedDataJson=async(p,n,f)=>s.events.filter(f).slice(0,n);`:`export const readCrmUsers=async()=>globalThis.__activity.users;`,loader:'js'}));}}]});
 const m={exports:{} as any};new Function('require','module','exports',r.outputFiles[0].text)(require,m,m.exports);
 try{const visible=await m.exports.readCrmActivity(state.users[1]);assert.deepEqual(visible.map((e:any)=>e.id),['1']);assert.equal(visible[0].actor.name,'Admin');assert.equal(visible[0].target.name,'Manager');assert.equal((await m.exports.readCrmActivity(state.users[0])).length,3);state.leads[0].assignedManagerId='a';assert.equal((await m.exports.readCrmActivity(state.users[1])).length,0);assert.equal((await m.exports.readCrmActivity(state.users[0],30,'2026-09-26T09:00:00Z'))[0].id,'3');state.leads[0]={...state.leads[0],archivedAt:'2026-09-26',status:'spam',statusHistory:[{status:'new',changedAt:'2026-09-25T08:00:00Z'},{status:'spam',changedAt:'2026-09-26T11:00:00Z'}]};state.events.unshift({id:'4',createdAt:'2026-09-26T11:00:00Z',type:'lead_status_changed',title:'Изменён статус заявки',managerId:'a',leadId:'mine',href:'/crm/leads'});const event=(await m.exports.readCrmActivity(state.users[0]))[0];assert.match(event.href,/id=mine&view=archive#mine/);assert.match(event.title,/Admin.*Спам/);assert.equal(event.changes[0].before,'Новая');assert.equal(event.target,undefined);}finally{delete (globalThis as any).__activity;}
});
