...
test("Georgia Yandex v2 audits six populated markets before recovery and all six after persistence", () => {
  ...
  assert.equal((workflow.match(/CATALOG_AUDIT_ASSERT_MARKETS: korea,china,japan,uae,europe$/gm) || []).length, 3);
  assert.equal((workflow.match(/CATALOG_AUDIT_ASSERT_MARKETS: korea,china,japan,uae,europe,georgia$/gm) || []).length, 1);
