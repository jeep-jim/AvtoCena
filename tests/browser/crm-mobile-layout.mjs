 ...
 for(const theme of ['dark','light'])for(const width of [320,390,768,1440])for(const kind of (process.env.CRM_TEST_PAGES?.split(',')||['overview','leads','team','settings','clients','client','archive','documents'])){
  ...
  try{
   ...
   if(kind==='documents'){
    ...
    await page.getByRole('button',{name:'Страна договора: Япония',exact:true}).click();assert.equal(await page.locator('.contract-market-menu svg').count(),6);await page.getByRole('button',{name:'Китай',exact:true}).click();assert.equal(await page.locator('.contract-country svg').count(),2);await page.getByRole('button',{name:'Страна договора: Китай',exact:true}).click();await page.getByRole('button',{name:'Япония',exact:true}).click();await page.getByRole('button',{name:'Создать договор',exact:true}).click();await page.locator('.contract-editor-layout').waitFor();await page.getByLabel('Номер договора',{exact:true}).fill('TEST/99');assert.ok((await page.locator('.contract-paper').innerText()).includes('TEST/99'));assert.equal(await page.locator('.contract-paper small').count(),0);
