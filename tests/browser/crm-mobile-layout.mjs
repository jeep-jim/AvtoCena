 ...
 for(const theme of ['dark','light'])for(const width of [320,390,768,1440])for(const kind of (process.env.CRM_TEST_PAGES?.split(',')||['overview','leads','team','settings','clients','client','archive','documents','staff'])){
  ...
  try{
   ...
   if(kind==='staff'){
    ...
    await page.getByRole('button',{name:'Просмотреть Кадровый документ.pdf',exact:true}).waitFor();page.once('dialog',d=>d.dismiss());await page.getByRole('button',{name:'Просмотреть Кадровый документ.pdf',exact:true}).click();assert.equal(await page.locator('[data-pdf-page]').count(),0);
    page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'Просмотреть Кадровый документ.pdf',exact:true}).click();await page.locator('[data-pdf-page] canvas[data-rendered=true]').waitFor();page.once('dialog',d=>d.dismiss());await page.getByRole('link',{name:'Скачать',exact:true}).click();await page.getByRole('button',{name:'Закрыть предпросмотр',exact:true}).click();
    page.once('dialog',d=>d.dismiss());await page.getByRole('button',{name:'Удалить Кадровый документ.pdf',exact:true}).click();assert.equal(staffDocs.length,1);page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'Удалить Кадровый документ.pdf',exact:true}).click();await page.getByText('Документов пока нет.',{exact:true}).waitFor();
