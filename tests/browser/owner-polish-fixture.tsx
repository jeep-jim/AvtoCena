...
import {UnavailableOfferView} from '../../apps/web/components/catalog/UnavailableOfferView';
...
function App(){return <CarsLayout><PublicHeader/><main style={{paddingTop:96}} className="ac-offer-page mx-auto max-w-[1500px] p-4 md:p-8"><h1 className="text-3xl font-black">HONDA STEPWGN 7SEATS</h1><CitySelector value="" onChange={()=>{}}/><div className="mt-6 max-w-[960px]"><OfferDesktopActions {...favorite} position="below"/><OfferMobileActions {...favorite}/><div data-spec-desktop data-open="false"/><div style={{width:390,maxWidth:"100%"}}><OfferDesktopActions {...favorite}/></div>{staff?<OfferPdfButton offerId="qa-owner" draft={{year:'2026'}}/>:null}</div><div className="ac-mobile-filter-sheet mt-6 max-w-sm"><CatalogBrandMultiSelect value="" options={[{value:'Abarth',label:'Abarth'},{value:'Acura',label:'Acura'}]} contextQuery="" onChange={()=>{}}/></div><section data-green-price style={{maxWidth:400}}><PriceTrend panel label="Стоимость под ключ" offer={{id:"green-709602",sourceId:"akebono_green_japan_open",offerType:"fixed",market:"japan",auctionGrade:"S",totalRub:2308313,sourceCurrency:"JPY",calculationSnapshot:{currencyRate:{currency:"JPY",effectiveRate:.55,previousEffectiveRate:.56}}}}/></section><OfferContactActionsStyles/></main></CarsLayout>}
const crmLinks=[['/crm','Обзор'],['/crm/leads','Заявки'],['/crm/clients','Клиенты'],['/crm/managers','Команда и права'],['/crm/settings','Рынки и расчёт'],['/crm/dealers','Дилеры'],['/crm/telegram','Telegram'],['/crm/documents','Документы']] as const;
createRoot(document.getElementById('root')!).render(new URLSearchParams(location.search).has('unavailable')?<UnavailableOfferView offer={{id:'missing',make:'Honda',model:'Stepwgn',market:'japan',reason:'sold',removedAt:''}}><section className="ac-unavailable-alternatives"><h2>Другие автомобили Honda</h2></section></UnavailableOfferView>:new URLSearchParams(location.search).has('crm')?<CrmShellView title="Рынки и расчёт" subtitle="Настройки" activeHref="/crm/settings" user={{id:'owner',role:'owner',displayName:'Owner'}} links={[...crmLinks]} avatar="/logo/avtocena-mark-light.svg"><div/></CrmShellView>:<App/>);
