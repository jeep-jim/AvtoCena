...
const pages:any={overview:Overview,leads:Leads,team:Managers,settings:Settings,clients:Clients,client:Client};
...
void pages[kind]({searchParams:Promise.resolve(Object.fromEntries(new URLSearchParams(location.search))),params:Promise.resolve({id:"client-0"})}).then(resolve).then((tree:any)=>createRoot(document.getElementById('root')!).render(tree));
