...
try{
 for(const width of [320,390,1440])for(const theme of ['light','dark']){
  ...
  assert.match(await gallery.locator('img').first().getAttribute('src'),/-1280.webp$/);await gallery.getByRole('button',{name:'Закрыть',exact:true}).click();
  ...
  assert.equal(downloads.length,0,'preview must not download');assert.equal(context.pages().length,1,'preview stays on site without popup/native PDF navigation');
  ...
  assert.ok(await preview.locator('[data-pdf-page] a').count()>0,'document links retained');
  ...
  await checks.first().uncheck();await preview.locator('canvas[data-rendered=true]').waitFor();await checks.first().check();await preview.locator('summary').click();
