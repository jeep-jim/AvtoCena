...
function App(){if(kind.startsWith('alerts'))return <PublicHeader backHref="/cars"/>;
 ...
 <div style={{maxWidth:390,margin:'40px auto 0'}}>
  <InlineOfferParameters canSave={kind==='saved-admin'} savedCalculation={savedCalculation as any} sourcePriceOnly={kind==='missing-hybrid'} autoCalculate={kind==='missing-hybrid'} offerId="qa-attached" initial={initial} showCommercial={kind==='n1'} isPickup={kind==='n1'} price={kind==='missing-hybrid'?<SellerPrice offer={{market:'korea',sellerPriceRub:4333490}}/>:<div className="ac-offer-price-panel" style={{padding:16,borderRadius:20,background:'var(--ac-surface-2)'}}><span>{kind==='missing-hybrid'?'Цена продавца':'Проверка интерфейса'}</span><br/><strong style={{fontSize:28}}>4 333 490 ₽</strong></div>}>
