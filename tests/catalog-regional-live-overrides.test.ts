...
test("AUTO.GE legacy parser fixture remains deterministic but is not a production adapter", () => {
  ...
  assert.doesNotMatch(importer, /prepareSource\(autoGeorgiaStrictSource\)/);
...
test("strict regional adapters preserve ordering without AUTO.GE production replacement", () => {
  ...
  assert.ok(genericAt >= 0 && exactAt > genericAt && currentAt > exactAt && mashinaAt > currentAt);
  assert.equal(importer.indexOf("prepareSource(autoGeorgiaStrictSource)"), -1);
});
