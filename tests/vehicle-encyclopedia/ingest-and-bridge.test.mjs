...
test("ingestion is a validated dry run and preserves totals for an empty batch", async () => {
  ...
  assert.equal(prepared.totals.source, 928);
});
...
test("legacy preview is review-only and excludes seed and kW-only entities", async () => {
  ...
  assert.equal(variants.length, 118);
  assert.equal(report.excludedModels.length, 1594);
  assert.equal(report.excludedVariants.length, 19122);
  assert.equal(report.excludedVariants.filter((row) => row.reason === "entity_status_not_verified").length, 18979);
  assert.equal(report.excludedVariants.filter((row) => row.reason.includes("powerHp")).length, 143);
});
