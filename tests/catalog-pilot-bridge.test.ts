...
test('production bridge diagnostics admit only registered exact one-page read routes', () => {
  ...
  const autopapa = new URL('https://avtocena.com/api/internal/georgia-recovery-e2f913?source=autopapa&pages=1&startPage=1');
  assert.equal(isExistingPilotBridgeRequest(autopapa, 'GET', 'autopapa_georgia_open'), true);
  assert.equal(isExistingPilotBridgeRequest(autopapa, 'GET', 'myauto_georgia_list'), false);
  const dubizzle = new URL('https://avtocena.com/api/internal/dubizzle-egress-a4c907?page=1');
  assert.equal(isExistingPilotBridgeRequest(dubizzle, 'GET', 'dubizzle_uae_open'), true);
  assert.equal(isExistingPilotBridgeRequest(dubizzle, 'POST', 'dubizzle_uae_open'), false);
});
